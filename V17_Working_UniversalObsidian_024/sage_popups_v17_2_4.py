# -*- coding: utf-8 -*-
"""
sage_popups.py — 팝업 다이얼로그 v17.2.3
[v17.2.3 안정화: 2026-06-08]
- V17.2.2 기준 안정화 복사본에서 작업
- 현재 파트 키 정규화 보강
- 하드코딩된 app_v17_1_4C 의존 제거
- Raw/Wiki/Schema 저장 보강
"""
...
# ─── V17.2.3 안전 헬퍼 ─────────────────────────────────────────────
def _normalize_part_key(raw_part: str) -> str:
    """사이드바 표시명(예: '파트 4: 이미지 생성')을 내부 part_key로 정규화한다."""
    raw = str(raw_part or "part1").strip()
    if raw.startswith("파트 0") or raw == "part0":
        return "part0"
    if raw.startswith("파트 1") or raw == "part1":
        return "part1"
    if raw.startswith("파트 2") or raw == "part2":
        return "part2"
    if raw.startswith("파트 3") or raw in ("part3", "part34"):
        return "part34"
    if raw.startswith("파트 4") or raw in ("part4", "part5img"):
        return "part5img"
    if raw.startswith("파트 5") or raw == "part5":
        return "part5"
    if raw.startswith("파트 6") or raw == "part6":
        return "part6"
    if raw.startswith("파트 7") or raw == "part7":
        return "part7"
    if raw.startswith("파트 8") or raw == "part8":
        return "part8"
    return raw


def _safe_call_app_save_workspace_state() -> bool:
    """팝업에서 현재 실행 중인 앱의 save_workspace_state를 안전하게 호출한다.
    하드코딩된 과거 모듈명(app_v17_1_4C)을 import하지 않는다.
    """
    try:
        import sys
        for module_name, module in list(sys.modules.items()):
            if hasattr(module, "save_workspace_state") and (
                module_name == "__main__" or module_name.startswith("app_v")
            ):
                module.save_workspace_state()
                return True
    except Exception:
        return False
    return False

# 범용 15개 카테고리 태그 분류 시스템 (다채널 운영용)
UNIVERSAL_CATEGORY_TAGS = {
    "심리학": ["심리", "자존감", "트라우마", "인지", "행동", "무의식", "불안", "감정", "방어기제", "상담", "치료"],
    "철학": ["쇼펜하우어", "스토아", "실존주의", "동양철학", "니체", "소크라테스", "칸트", "형이상학", "윤리", "사상"],
    "종교": ["성경", "불교", "신앙", "영성", "기독교", "이슬람", "명상", "기도", "신학"],
    "역사": ["인물", "사건", "시대", "문명", "고대", "중세", "근대", "현대", "전쟁", "왕조", "제국"],
    "과학": ["물리", "화학", "생물", "수학", "천문", "우주", "진화", "유전", "양자", "뇌과학"],
    "기술": ["AI", "프로그래밍", "소프트웨어", "하드웨어", "인공지능", "코딩", "데이터", "네트워크", "알고리즘", "로봇"],
    "경제·비즈니스": ["투자", "창업", "시장", "재무", "마케팅", "경영", "주식", "부동산", "금융", "화폐"],
    "사회": ["정치", "문화", "인구", "도시", "제도", "정책", "불평등", "인권", "미디어", "복지"],
    "교육": ["학습", "교수법", "자기계발", "독서", "학교", "강의", "공부", "기억", "성장"],
    "건강": ["의학", "운동", "영양", "수면", "노화", "질병", "식단", "다이어트", "멘탈", "예방"],
    "예술·문학": ["소설", "시", "영화", "음악", "미술", "디자인", "건축", "연극", "공연", "작품"],
    "미디어·콘텐츠": ["유튜브", "블로그", "팟캐스트", "SNS", "크리에이터", "콘텐츠", "조회수", "구독", "플랫폼"],
    "법률": ["법", "규제", "계약", "지적재산", "소송", "재판", "헌법", "민법", "형법", "저작권"],
    "환경": ["기후", "생태", "에너지", "지속가능", "탄소", "오염", "재활용", "자연", "보존"],
    "기타": ["기타", "일반", "미분류", "잡학", "상식"],
}
...
PART_CONTEXT_MAP = {
    ...
    "part5": {
        "name": "Part 5 — Video / Opal Dispatch (영상 생성 분배)",
        "keys": ["p6_video_mapped_result", "p6_video_valid_rows", "p6_video_opal_data"],
        "desc": "Google Opal용 8초 영상 프롬프트, 계정 분배 JSON, 누락/재시도 관리 파트",
        "obsidian_folder": "Part5_Video",
    },
    "part5img": {
        "name": "Part 4 — Image / Google Flow + Auto Flow (이미지 운영 패키지)",
        "keys": ["p5_a_result", "p5_b_result", "p5_c_results", "p5_valid_rows", "p5_image_master_prompt"],
        "desc": "A/B/C 에셋, Google Flow + Auto Flow용 배치 프롬프트, 이미지 파일명 규칙 생성 파트",
        "obsidian_folder": "Part4_Image",
    },
    "part6": {
        "name": "Part 6 — Audio / TTS-ready / BGM",
        "keys": ["p34_narration_script", "p6_opal_data", "p6_bgm_selection", "p6_narration_verified"],
        "desc": "TTS-ready 대본, SSML, 무음 구간, BGM 큐시트 정리 파트",
        "obsidian_folder": "Part6_Audio",
    },
    "part7": {
        "name": "Part 7 — Shorts / Longform Funnel (숏폼)",
        "keys": ["p7_script_input", "p7_capcut_data", "p7_capcut_data_v2", "p7_shortform_hook"],
        "desc": "롱폼 핵심 감정을 60초 숏폼으로 재구성하고 유입 문장을 만드는 파트",
        "obsidian_folder": "Part7_Shorts",
    },
    "part8": {
...
def _get_current_part() -> str:
    """현재 선택된 파트를 세션 스테이트에서 읽고 내부 키로 정규화한다."""
    return _normalize_part_key(st.session_state.get("sidebar_part", "part1"))
...
def _build_obsidian_rag_context() -> str:
    ...
    vault_root = Path(st.session_state.get("path_obsidian", r"C:\SageMirror_Production\00_Obsidian"))
...
def _save_raw_wiki(user_query, content, title, source_type, part_key, model_name):
    """Raw/Wiki 이중 저장 — 00_Raw(원본) + 01_Wiki(개념추출) + 02_Schema + 03_Logs"""
    try:
        import hashlib
        import json as _json
        
        vault = r"C:\SageMirror_Production\00_Obsidian"
        ...
        combined_text = f"Q: {user_query}\n\nA: {content}" if user_query else content
        content_hash = hashlib.sha256(combined_text.encode("utf-8")).hexdigest()
        
        logs_dir = _P(vault) / "03_Logs"
        dup_log = logs_dir / "Duplicates" / f"{today_date}.jsonl"
        import_log = logs_dir / "Import" / f"{today_date}.jsonl"
        
        # Check for duplicates using Schema files
        schema_dir = _P(vault) / "02_Schema" / "Items"
                ...
                try:
                    with open(s_file, "r", encoding="utf-8") as f:
                        ...
                        if data.get("content_hash") == content_hash:
                            dup_dir = dup_log.parent
                            dup_dir.mkdir(parents=True, exist_ok=True)
                            log_entry = {
                                "timestamp": today,
                                "action": "duplicate_skip",
                                "content_hash": content_hash,
                                "existing_path": str(s_file),
                                "created_by": "v17_right_research"
                            }
                            with open(dup_log, "a", encoding="utf-8") as lf:
                                lf.write(_json.dumps(log_entry, ensure_ascii=False) + "\n")
                            return {"status": "duplicate", "message": f"중복: {s_file.name}"}
                except Exception:
                    continue

        # Raw 저장 (원본 그대로) - Conversations
        raw_dir = _P(vault) / "00_Raw" / "Conversations" / yyyy / mm
        raw_dir.mkdir(parents=True, exist_ok=True)
        raw_path = raw_dir / f"원본대화_{ts}_{content_hash[:8]}.md"
        ...
        # 카테고리 추출
        try:
            tags_dict = _classify_universal_tags(combined_text + " " + title)
            cats = list(tags_dict.keys())
            if not cats:
                cats = ["기타"]
        except Exception:
            tags_dict = {}
        ...
        # Wiki 저장 (개념 추출)
        wiki_dir = _P(vault) / "01_Wiki" / main_cat
        ...
        try:
            flat_tags = list(set([t for cat_tags in tags_dict.values() for t in cat_tags]))
            tag_links = " ".join([f"[[{t}]]" for t in flat_tags[:5]])
            tag_hashes = " ".join([f"#{t.replace(' ', '_')}" for t in flat_tags[:5]])
        except Exception:
            tag_links = ""
            tag_hashes = ""
            flat_tags = []
...
- 소스 유형: {source_type}
- 모델: {model_name}
- Raw 원본: [[00_Raw/Conversations/{yyyy}/{mm}/{raw_path.name}]]
...
"""
        ...
        # Schema 저장 (검색/재사용용 구조화 메타데이터)
        schema_dir.mkdir(parents=True, exist_ok=True)
        schema_path = schema_dir / f"{safe}_{ts}.json"
        schema_data = {
            "source_id": f"{safe}_{ts}",
            "title": title,
            ...
            "keywords": flat_tags,
            "language": "ko",
            "raw_path": f"00_Raw/Conversations/{yyyy}/{mm}/{raw_path.name}",
            "wiki_path": f"01_Wiki/{main_cat}/{wiki_path.name}",
            "content_hash": content_hash,
            "original_url": "",
        ...
        schema_path.write_text(_json.dumps(schema_data, ensure_ascii=False, indent=2), encoding="utf-8")

        # Import 로그 기록
        import_dir = import_log.parent
        ...
        log_entry = {
            ...
            "raw_path": f"00_Raw/Conversations/{yyyy}/{mm}/{raw_path.name}",
            "wiki_path": f"01_Wiki/{main_cat}/{wiki_path.name}",
            "schema_path": f"02_Schema/Items/{schema_path.name}",
            "content_hash": content_hash,
            "created_by": "v17_right_research"
        }
        with open(import_log, "a", encoding="utf-8") as lf:
            lf.write(_json.dumps(log_entry, ensure_ascii=False) + "\n")

        return {"status": "success", "raw": str(raw_path), "wiki": str(wiki_path), "schema": str(schema_path)}
    except Exception as e:
        err_msg = f"[Raw/Wiki 저장 오류] {e}"
        try:
            vault = r"C:\SageMirror_Production\00_Obsidian"
            from pathlib import Path as _P
            err_log = _P(vault) / "03_Logs" / "Errors" / f"{datetime.now().strftime('%Y-%m-%d')}.jsonl"
            err_log.parent.mkdir(parents=True, exist_ok=True)
            import json as _json
            with open(err_log, "a", encoding="utf-8") as lf:
                lf.write(_json.dumps({"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "action": "error", "error": str(e), "created_by": "v17_right_research"}, ensure_ascii=False) + "\n")
        except: pass
...
"""
        ...
        # 저장 경로 결정
        save_dir = Path(st.session_state.get("path_obsidian", r"C:\SageMirror_Production\00_Obsidian")) / obsidian_folder
        ...
        tag_index_dir = Path(st.session_state.get("path_obsidian", r"C:\SageMirror_Production\00_Obsidian")) / "TagIndex"
        ...
        # V17.2.3: 범용 기억 시스템 보강 — 같은 내용을 Raw/Wiki/Schema에도 동시 저장
        try:
            _save_raw_wiki("", content_md, title, source_type, part_key, model_name)
        except Exception:
            pass
...
def popup_edit_obsidian():
    ...
    with c1:
        if st.button("💾 저장", use_container_width=True, type="primary", key="ob_save"):
            ...
            try:
                _safe_call_app_save_workspace_state()
            except Exception:
                pass
            try:
                from rag_memory_utils import update_recent_activity_memory
...
def popup_settings_modal():
    ...
    with st.expander("🎨 세부 UI 색상", expanded=False):
        ...
        st.markdown("#### 2. 버튼 색상")
        button_background_color = st.color_picker("기본 버튼 배경색", value=app_settings.get("button_background_color", "#1E293B"), key="settings_btn_bg_color")
        button_text_color = st.color_picker("기본 버튼 글자색", value=app_settings.get("button_text_color", "#B8860B"), key="settings_btn_txt_color")
        button_border_color = st.color_picker("기본 버튼 테두리색", value=app_settings.get("button_border_color", "#B8860B"), key="settings_btn_brd_color")
        ...
        card_background_color = st.color_picker("본문 카드 배경색", value=app_settings.get("card_background_color", "#1E293B"), key="settings_card_bg_color")
        input_background_color = st.color_picker("입력창 배경색", value=app_settings.get("input_background_color", "#0F172A"), key="settings_inp_bg_color")
    ...
    with st.expander("🛠 고급 기능", expanded=False):
        ...
        st.markdown("### 시스템 자가진단")
        if st.button("🧪 시스템 자가진단 실행", use_container_width=True, key="settings_self_diag_button"):
            import os
            from pathlib import Path
            import json
            ...
            # Agent School 검사
            agent_school_state = st.session_state.get("agent_school", {})
            has_agent_school = bool(agent_school_state)
            part_count = len(agent_school_state.get("parts", {})) if has_agent_school else 0
            agent_count = sum(len(agents) for agents in agent_school_state.get("parts", {}).values()) if has_agent_school else 0
            packet_queue_len = len(agent_school_state.get("packet_queue", [])) if has_agent_school else 0
            
            # Prompt 폴더 검사
            has_prompts = (cwd / "prompts").exists()
            has_gemma_prompts = (cwd / "prompts" / "gemma").exists()
            has_part1_prompts = (cwd / "prompts" / "part1").exists()
            
            # Obsidian 설정 검사
            obs_path = st.session_state.get("path_obsidian")
            has_obs_path = bool(obs_path)
            
            # API Key 보안 검사
            api_key_safe = "PASS"
            workspace_safe = "PASS"
            sensitive_keys = ["api_key", "token", "PAT", "tavily_api_key", "youtube_api_key", "gemini_api_key", "github_token", "openai_api_key", "anthropic_api_key"]
            
            ws_file = cwd / "workspace_state.json"
            if ws_file.exists():
                try:
            ...
            ls_file = cwd / "local_secrets.json"
            if ls_file.exists():
                gi_file = cwd / ".gitignore"
                if gi_file.exists():
                    try:
                        with open(gi_file, "r", encoding="utf-8") as f:
                            if "local_secrets.json" not in f.read():
                                api_key_safe = "WARNING"
                    except:
                        pass
                else:
                    api_key_safe = "WARNING"
            ...
            results = [
                ("| 항목 | 결과 | 설명 |"),
                ("| --- | --- | --- |"),
                (f"| 현재 폴더 | {cwd.name} | {cwd} |"),
                (f"| Agent School | {'PASS' if has_agent_school else 'FAIL'} | Parts: {part_count}, Agents: {agent_count} |"),
                (f"| Packet Queue | PASS | Queue count: {packet_queue_len} |"),
                (f"| Prompt MD | {'PASS' if has_prompts and has_gemma_prompts and has_part1_prompts else 'FAIL'} | prompts, gemma, part1 확인 |"),
                (f"| Obsidian 설정 | {'PASS' if has_obs_path else 'FAIL'} | |"),
                (f"| API Key 보안 | {api_key_safe} | 화면 노출 여부 / .gitignore 확인 |"),
                (f"| workspace_state 보안 | {workspace_safe} | 민감정보 저장 여부 확인 |")
            ]
...
def popup_assistant():
    ...
    # ── 이전 클릭에서 저장된 입력 처리 ──────────────
    if st.session_state.get("_popup_pending_input"):
        _pending = st.session_state._popup_pending_input
        st.session_state._popup_pending_input = None
        with st.spinner("⚙️ 생성 중..."):
            _full_resp = call_gemma(
                prompt=_pending,
                system=st.session_state.get("_popup_last_sys", ""),
                model=st.session_state.get("_popup_last_model", "gemma4:e2b")
            )
        if _full_resp:
            st.session_state.popup_history.append({
                "role": "assistant",
                "content": _full_resp,
                "model": st.session_state.get("_popup_last_model", "gemma4:e2b"),
                "part": st.session_state.get("sidebar_part", "part1")
            })
...
div[data-testid="stSelectbox"] > div {
    ...
    background-color: #1a1a21 !important;
        ...
        if _send and _prompt and _prompt.strip():
            ...
            st.session_state._popup_pending_input = _prompt
            st.session_state._popup_last_sys = _sys
            st.session_state._popup_last_model = _cur_model
