# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's Mirror Studio) — Master App v7.7
================================================================
[v7.7 업데이트 사항: 2026-05-11 18:30]
- UI 개편: 젬마 프로토콜 팝업(스크롤, 복붙 가능) 적용
- 자동화: 채널 검색 시 떡상 원본 채널 1개를 찾아 URL 자동 매칭
- 프롬프트: 200개 댓글(체험담, 공감) 참조 가상 분석 로직 추가
- UI 오류 수정: 하단 3칸 텍스트 영역 클릭 및 복붙 불가(disabled) 해제 및 구분선 추가
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v7.7",
    page_icon="🪞",
    layout="wide",
...
# 2. 세션 상태 초기화 (v7.7)
# =====================================================================
DEFAULT_OBSIDIAN_RULES_V77 = """[옵시디언 지식 구조화 규칙서]
...
MASTER_RESEARCH_PROMPT_V77 = """[자료 조사 파트 전용 마스터 규칙서: 절대 가이드]
...
GEMMA_PROTOCOL_V77 = """[Gemma4 행동 및 작업 지침서 (Gemma Protocol)]
...
def init_session_state():
    defaults = {
        ...
        "tavily_api_key": "",
        "obsidian_rules": DEFAULT_OBSIDIAN_RULES_V77,
        "base_prompt_rules": MASTER_RESEARCH_PROMPT_V77,
        "obsidian_history": [],
        "prompt_history": [],
        "popup_history": [],
        ...
        "p1_gemma_protocol": GEMMA_PROTOCOL_V77,
    ...
    if st.session_state.base_prompt_rules != MASTER_RESEARCH_PROMPT_V77 and not st.session_state.get("p1_saved"):
        st.session_state.base_prompt_rules = MASTER_RESEARCH_PROMPT_V77
    if st.session_state.obsidian_rules != DEFAULT_OBSIDIAN_RULES_V77 and not st.session_state.get("p1_saved"):
        st.session_state.obsidian_rules = DEFAULT_OBSIDIAN_RULES_V77
    if st.session_state.p1_gemma_protocol != GEMMA_PROTOCOL_V77 and not st.session_state.get("p1_saved"):
        st.session_state.p1_gemma_protocol = GEMMA_PROTOCOL_V77
...
# =====================================================================
def render_login():
    st.markdown(f"<h1 style='text-align:center'>{APP_TITLE} <span style='color:#10B981;font-size:0.5em;'>v7.7 Flow Edition</span></h1>", unsafe_allow_html=True)
...
def render_part1():
    ...
    with c_left:
        ...
        # v7.7: 클릭/복붙 가능하도록 disabled 제거. 대신 readonly 역할을 위해 st.text_area 사용 (편집은 팝업에서)
    ...
    with c_right:
        ...
        st.caption("AI 카피를 배제한 순수 인간 운영의 최고 조회수 채널을 자동 매칭합니다.")
        search_kw = st.text_input("검색 키워드 (가이드라인 기반 자동 세팅)", value="50대 심리 철학 위로 채널", disabled=is_locked)
        
        if st.button("🌐 전 세계 채널 검색 및 자동 매칭 (Tavily)", disabled=is_locked, use_container_width=True):
            if not st.session_state.tavily_api_key:
                st.error("좌측 사이드바 '⚙️ 설정 변경'에서 Tavily API Key를 먼저 입력하세요.")
            else:
                with st.spinner("최고 떡상 원본 채널을 탐색 및 매칭 중..."):
                    from sage_engine import tavily_search
                    # v7.7: 프롬프트 지침에 맞춘 강력한 쿼리
                    q = search_kw + " highest views human operated psychology philosophy -AI -auto youtube channel"
                    res = tavily_search(q, st.session_state.tavily_api_key, max_results=1) # 가장 강력한 1개 추출
                    
                    if "error" in res: st.error(res["error"])
                    else:
                        st.session_state.p1_channel_search_results = res.get("results", [])
                        if st.session_state.p1_channel_search_results:
                            top_url = st.session_state.p1_channel_search_results[0].get('url', '')
                            if top_url:
                                st.session_state.p1_channel_url = top_url
                                st.success("🎯 최고의 타겟 채널이 성공적으로 자동 매칭되었습니다!")
        
        if st.session_state.p1_channel_search_results:
            with st.container(border=True):
                r = st.session_state.p1_channel_search_results[0]
                st.markdown(f"**매칭된 채널:** [{r.get('title', '제목없음')}]({r.get('url', '#')})")
        
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("##### 🎯 분석 대상 확정")
        st.session_state.p1_channel_url = st.text_input("타겟 유튜브 URL (자동 입력됨)", value=st.session_state.p1_channel_url, disabled=is_locked)
    ...
    with c_research:
        ...
        if st.session_state.p1_research_result:
            # v7.7 disabled 제거하여 복사/붙여넣기 및 스크롤 가능하게 처리
            st.text_area("자료 조사 결과 (복사 가능)", value=st.session_state.p1_research_result, height=450, label_visibility="collapsed")
    ...
    with c_plan:
        ...
        if st.session_state.p1_planning_result:
            # v7.7 disabled 제거하여 복사/붙여넣기 및 스크롤 가능하게 처리
            st.text_area("최종 기획안 (복사 가능)", value=st.session_state.p1_planning_result, height=350, label_visibility="collapsed")
            ...
            if st.button("🔒 최종안 옵시디언 자동 백업", type="primary", use_container_width=True):
                ...
                if st.session_state.path_obsidian:
                    ...
                    md = f"""# [[{topic_title}]]
## 📌 Brief Summary
Sage Mirror Studio v7.7 총괄 시나리오 기획안
...
- **Projects/Contexts:** [[SageMirror_Production_v7.7]]
