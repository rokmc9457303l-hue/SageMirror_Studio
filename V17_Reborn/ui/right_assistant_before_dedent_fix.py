...
def ensure_right_assistant_state() -> None:
    """우측 대화창 세션 기본값."""
    version_key = "right_assistant_ui_version"
    current_version = "2026-06-10-clean-v1"
    ...
    if "right_assistant_messages" not in st.session_state or not st.session_state["right_assistant_messages"]:
        st.session_state["right_assistant_messages"] = [
            {
                "role": "system",
                "content": (
        ...
        bad_keywords = ["<div", "</div>", "sage-right-input", "sage-chat-card", "sage-input-mock", "unsafe_allow_html"]
        clean_msgs = []
        for msg in st.session_state["right_assistant_messages"]:
            content_str = str(msg.get("content", ""))
            if any(keyword in content_str for keyword in bad_keywords):
                continue
        ...
        st.session_state["right_assistant_messages"] = clean_msgs
