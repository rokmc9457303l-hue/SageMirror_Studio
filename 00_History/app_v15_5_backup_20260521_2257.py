# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 — Master App v15.5
[v15.5 업데이트 사항: 2026-05-21]
- 시스템 동기화 카드의 가로 균등 4열 배치(비율 1:1:1:1) 및 로컬 DB 카드와의 디자인 통일화
- 8개 전 파트 헤더에 Gemma 모델 선택기(GEMMA4:E2B / GEMMA4:E4B) 탑재 및 올라마 실시간 연동 개발
"""
...
def render_top_panel():
    ...
    .clickable-card-wrapper button {
        background-color: rgba(30, 41, 59, 0.45) !important;
        border: 1px solid rgba(16, 185, 129, 0.4) !important; /* 기본성공테두리: 초록 */
        color: #f5e9d3 !important;
        height: 60px !important;
        text-align: left !important;
    ...
    /* 개별 카드 특화 보더색 지정 */
    .clickable-card-wrapper.card-rag-btn button {
        border-color: rgba(16, 185, 129, 0.45) !important;
    }
    .clickable-card-wrapper.card-git-btn button {
        border-color: rgba(16, 185, 129, 0.45) !important;
    }
    .clickable-card-wrapper.card-sync-btn button {
        border-color: rgba(212, 175, 106, 0.5) !important; /* 동기화 버튼 골드 테두리 */
    }
    
    /* 경고/에러 상태 테두리 */
    .clickable-card-wrapper.card-rag-warn button {
        border-color: rgba(245, 158, 11, 0.45) !important; /* 오렌지 */
    }
    .clickable-card-wrapper.card-git-err button {
        border-color: rgba(239, 68, 68, 0.45) !important; /* 레드 */
    }
...
# =====================================================================
def render_part1():
    c_title, c_control = st.columns([5.5, 4.5])
    
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">📚 파트 1: 벤치마킹 & 자료조사</h3>', unsafe_allow_html=True)
        
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
...
def render_part2():
    ...
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">🎨 파트 2: 총괄기획</h3>', unsafe_allow_html=True)
...
def render_part5_image():
    c_title, c_control = st.columns([5.5, 4.5])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">🖼️ 파트 4: 이미지 생성</h3>', unsafe_allow_html=True)
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
...
def render_part6_video():
    c_title, c_control = st.columns([5.5, 4.5])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">🎥 파트 5: 영상 생성</h3>', unsafe_allow_html=True)
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
...
def render_part34():
    ...
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">✍️ 파트 3: 대본 작성</h3>', unsafe_allow_html=True)
...
def render_part6_opal():
    c_title, c_control = st.columns([5.5, 4.5])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">🎵 파트 6: 나레이션 & 배경음악</h3>', unsafe_allow_html=True)
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
...
def render_part7_capcut():
    c_title, c_control = st.columns([5.5, 4.5])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">🎬 파트 7: 숏폼 생성 (CapCut Bridge)</h3>', unsafe_allow_html=True)
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
...
def render_part8_dashboard():
    c_title, c_control = st.columns([5.5, 4.5])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 52px; color: #d4af6a; font-size: 1.65em; font-weight: 700;">📊 파트 8: 캡컷 최종 조립</h3>', unsafe_allow_html=True)
    with c_control:
        st.markdown('<div id="header-control-box-anchor"></div>', unsafe_allow_html=True)
        c_pin_in, c_pop_in = st.columns([6, 4])
        with c_pin_in:
