...
🪞 현자의 거울 스튜디오 — Master App v15.9.9

[v15.9.9 업데이트 사항: 2026-05-22]

- 전역 자동 웹 리서치 루프(Self-Research Loop) 도입 (최대 4회)

- RAG 지식 부족 시 Tavily 웹 검색 자동 보완 체인 연동
...
# ── call_gemma 래핑 함수 (Gemma4:E2B / GEMMA4:E4B 실시간 모델 스위칭 연동 + 최대 4회 자동 웹 검색 루프) ──
...
def call_gemma(prompt, system="", model=None):
    ...
    for loop_idx in range(max_loops):
        ...
        api_key = st.session_state.get("tavily_api_key", "").strip()

        if not api_key:

            st.warning("⚠️ Gemma가 추가 리서치[NEED_RESEARCH]를 요구했으나, Tavily API Key가 설정되지 않았습니다. 현재 자료를 기반으로 답변을 완성합니다.")

            fallback_prompt = f"{current_prompt}\n\n[알림: Tavily 인터넷 검색 API 키가 설정되지 않았습니다. 현재 입력 데이터와 옵시디언 RAG 정보만을 바탕으로 최대한 상상(환각) 없이 사실 위주로 최종 결과물을 작성하고, 출처가 불확실한 부분은 '출처 미확인'으로 기재하십시오.]"

            res = _orig_call_gemma(fallback_prompt, system=current_system, model=model)

            return res

        st.toast(f"🌐 젬마가 추가 웹 리서치를 감지했습니다: '{keyword}' ({loop_idx + 1}/{max_loops}회차)", icon="🔍")

        try:

            from sage_engine import tavily_search

            search_res = tavily_search(keyword, api_key, max_results=5)

            if "error" in search_res:

                st.error(f"❌ Tavily 검색 실패: {search_res['error']}")
            ...
            research_context = (
                ...
                f"반드시 인용된 웹 정보의 출처를 `[SOURCE: 웹 — {results[0].get('url', 'URL 미상')} — 검색일: {today_str}]` 와 같이 구체적인 URL과 함께 명시하라.\n"
            ...
            st.error(f"❌ 자동 리서치 수행 중 오류 발생: {e}")
