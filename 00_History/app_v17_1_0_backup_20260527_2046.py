...
- [수술 D] 모드 토글(A 일반 대화 / B 옵시디언 아카이빙) + 로컬 파일 경로 직접 읽기 UI 추가
...
def save_workspace_state():
    try:
        ...
        # DataFrame인 키들은 dict 변환 또는 빈 리스트 처리 후 JSON 직렬화
        # NaN 값을 None으로 치환하여 JSON 에러 방지
        for k, val in list(state_dict.items()):
            if isinstance(val, pd.DataFrame):
                val = val.replace({np.nan: None})
                state_dict[k] = val.to_dict(orient="records")
