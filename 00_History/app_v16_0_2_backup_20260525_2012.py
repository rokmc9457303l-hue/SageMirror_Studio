...
🪞 현자의 거울 스튜디오 — Master App v16.0.2

[v16.0.2 업데이트 사항: 2026-05-25]
- [A] RAG Memory 관련 핵심 함수 5종을 순수 Python 모듈인 rag_memory_utils.py로 분리 추출
- [B] rag_memory_utils.py에서 Streamlit 관련 종속성(st.*) 전면 배제 및 sys.stderr 예외 처리 도입
- [C] build_condensed_reference_context()의 오염/제외 목록 반환 튜플화 및 st.caption UI 출력 제어권을 sage_popups.py로 완전히 이관
- [D] app_v16_0_2.py 내부의 RAG 중복 정의 코드를 제거하고 rag_memory_utils import 구조로 안정화
- [E] RUN_APP.bat, RUN_DEBUG.bat, RUN_APP.vbs의 실행 타깃을 app_v16_0_2.py로 최신화 (8505 포트 유지)
