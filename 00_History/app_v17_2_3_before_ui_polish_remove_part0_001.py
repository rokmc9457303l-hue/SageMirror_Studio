...
with st.sidebar:
    ...
    if cur_p.startswith("파트 0"): cur_p = part_options[0]
    idx = 0
...
def render_part0_assistant():
    ...
    with c_popup:
        if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p0_popup_btn"):
            pass # popup_assistant()  # 안내문 띄우지 말고 즉시 대화창(팝업)을 열 것!
...
def render_part1():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p1_popup_btn"):

                st.session_state.sidebar_part = "part1"

                pass # popup_assistant()
...
def render_part2():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p2_popup_btn"):
...
def render_part5():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p5_popup_btn"):
...
def render_part5_image():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p5img_popup_btn"):
...
def render_part6_video():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p6_vid_popup_btn"):
...
def render_part34():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p34_popup_btn"):
...
def render_part6_opal():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p6_popup_btn"):
...
def render_part7_capcut():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p7_popup_btn"):
...
def render_part8_dashboard():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if False: # st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p8_popup_btn"):
...
def render_right_gemma_panel():
    ...
    /* 제미나이 스타일 입력창 UI */
    div[data-testid="column"]:has(#right-gemma-sticky-marker) div[data-testid="stTextInput"] input {
        border-radius: 20px !important;
        background-color: #21212c !important;
        border: 1px solid #333 !important;
        padding-left: 15px !important;
    }
    div[data-testid="column"]:has(#right-gemma-sticky-marker) div[data-baseweb="select"] > div {
        border-radius: 20px !important;
        background-color: #21212c !important;
        border: 1px solid #333 !important;
    }
    div[data-testid="column"]:has(#right-gemma-sticky-marker) button[kind="primary"] {
        border-radius: 20px !important;
        background-color: #e50914 !important;
        color: white !important;
        border: none !important;
    }
    div[data-testid="column"]:has(#right-gemma-sticky-marker) button[data-testid="baseButton-secondary"]:has(div:contains("＋")) {
    ...
    # 2. 하단 입력 바 및 기능
    st.markdown("<br>", unsafe_allow_html=True)
    c_plus, c_input, c_model, c_send = st.columns([0.8, 5.8, 2.7, 0.9])
    ...
    with c_plus:
        with st.popover("＋", use_container_width=True):
            ...
            if st.button("📁 파일 탐색기", use_container_width=True):
                st.info("다음 단계에서 파일 탐색기 연동 예정입니다.")
            if st.button("☁️ Google Drive / Docs", use_container_width=True):
                st.info("다음 단계에서 Google Drive/Docs 연동 예정입니다.")
            st.markdown("---")
            st.markdown("🔎 **외부 리서치 (모드 변경용)**")
            if st.button("📺 유튜브 채널 찾기", use_container_width=True):
                st.info("다음 단계 연동 예정입니다.")
            ...
            if st.button("🧩 현재 Part로 Push", use_container_width=True):
                ...
                if last_assistant_msg:
                    ...
                    st.success("보강 자료가 전달 대기 상태가 되었습니다.")
                else:
                    st.warning("Push할 응답이 없습니다.")
            if st.button("📋 마지막 응답 보기/복사", use_container_width=True):
                last_assistant_msg = None
                for msg in reversed(st.session_state.right_popup_history):
                    if msg.get("role") == "assistant" and msg.get("source") != "사용자 입력":
                        last_assistant_msg = msg
    ...
    if send_clicked and user_input.strip():
        ...
        with chat_container.chat_message("assistant"):
            ...
            if new_model_id == "tavily":
                tavily_key = st.session_state.get("tavily_api_key", "").strip()
                if not tavily_key:
                    response = "⚠️ Tavily API Key가 설정되지 않았습니다."
                    st.error(response)
                    source_label = "Tavily 에러"
                else:
                    with st.spinner("인터넷 검색 중..."):
                        try:
                            search_res = run_tavily_research(user_input, tavily_key, max_results=5)
                            formatted_results = ""
                            if "results" in search_res:
                                for idx, r in enumerate(search_res["results"], 1):
                                    formatted_results += f"[{idx}] 제목: {r.get('title')}\n요약: {r.get('content')}\nURL: {r.get('url')}\n\n"
                            ...
                            search_enriched_prompt = f"질문: {user_input}\n\n[수집된 최신 검색 데이터]\n{formatted_results}"
