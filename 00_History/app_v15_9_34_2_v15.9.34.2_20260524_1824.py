...
def clean_prompt_contamination(text: str) -> str:
    ...
    pattern = _re.compile(r'<span title="리서치 자동저장 대기 중".*?</span>', _re.DOTALL)
    ...
    for k in keys_to_save:
        ...
        if k in st.session_state:
            ...
            if k in contam_keys and isinstance(val, str):
                val = clean_prompt_contamination(val)
...
P2_MASTER_PROMPT_DEFAULT = """# 🎬 현자의 거울 — Part 2 Alchemist 전역 마스터 프롬프트 v1.0

너는 현자의 거울 스튜디오의 연금술사(Alchemist)이다.
Part 1이 수집한 원석 데이터를 다크심리학→철학→에세이→성경 순으로 변환하라.

# 🎯 핵심 3원칙
HOW → 자유 | WHAT → 통제 | WHO → 고정(@Protagonist)

# 🌑 기(起): 반드시 다크심리학으로 시작
가스라이팅·나르시시즘·조종·정서적방치·의존성심화
부족 시: [NEED_RESEARCH: 다크심리학 {emotion}]

# 📚 RAG 우선순위
1.[READ_OBSIDIAN:키워드] 2.성경 3.ResearchMemory
4.철학원문 5.다크심리학[NEED_RESEARCH] 6.웹검색 7.Gemma추론

# ⛔ 절대 금지
- 없는 성경구절 생성 / 존재하지 않는 인용
- AI냄새 문장 / 자기계발 말투 / 대본·나레이션 작성

# ✊ @Protagonist: 가르치지않고 통찰만 / 위로안하고 고통인정 / 침묵과여백 유지
"""
...
def render_top_panel():
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 마스터 프롬프트", expanded=False):
        ...
        with L:
            ...
            if "top_ob_view_widget" not in st.session_state:
                st.session_state.top_ob_view_widget = st.session_state.get("obsidian_rules", "")

            st.text_area("옵시디언 규칙서", value=st.session_state.top_ob_view_widget, height=180, key="top_ob_view_widget", label_visibility="collapsed")

            if st.button("💾 옵시디언 규칙서 저장", key="top_ob_save_btn", use_container_width=True):
                st.session_state.obsidian_rules = st.session_state.top_ob_view_widget
                save_workspace_state()
            ...
            with research_cols[2]:
                ...
                led_text = "🟢" if is_saved else "⚪"

                led_tooltip = "리서치 자동저장 완료" if is_saved else "리서치 자동저장 대기 중"

                st.markdown(f"<div style='text-align: center; padding-top: 8px;' title='{led_tooltip}'>{led_text}</div>", unsafe_allow_html=True)
            ...
            with btn_cols[2]:
                ...
                with st.popover("📂 저장 확인", use_container_width=True):
                    ...
                    auto_save_active = st.session_state.get("research_auto_save", True)
                    is_saved = st.session_state.get("research_auto_save_success", False)
                    auto_save_str = "ON" if auto_save_active else "OFF"
                    last_save_status = "성공" if is_saved else "대기 중"
                    
                    st.markdown(f"✅ 리서치 자동저장 상태: **{auto_save_str}**")
                    st.markdown(f"✅ 마지막 저장 상태: **{last_save_status}**")
                    st.divider()
        ...
        with R:
            ...
            if f"top_pr_view_{prompt_key}_widget" not in st.session_state:
                st.session_state[f"top_pr_view_{prompt_key}_widget"] = st.session_state.get(prompt_key, "")

            st.text_area("파트 마스터 프롬프트", value=st.session_state[f"top_pr_view_{prompt_key}_widget"], height=180, key=f"top_pr_view_{prompt_key}_widget", label_visibility="collapsed")
            ...
            with pr_btn_cols[1]:
                if st.button("💾 마스터 프롬프트 저장", key=f"pr_save_btn_{prompt_key}", use_container_width=True):
                    st.session_state[prompt_key] = st.session_state[f"top_pr_view_{prompt_key}_widget"]
                    save_workspace_state()
    ...
    with c1:
        ...
        if st.button("💾 저장 및 닫기", type="primary", use_container_width=True, key=f"popup_edit_val_save_{session_key}"):
            ...
            if session_key == "obsidian_rules":
                ...
                st.session_state[f"top_pr_view_{session_key}_widget"] = new_val
