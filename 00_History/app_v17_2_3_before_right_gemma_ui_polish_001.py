...
def render_right_gemma_panel():
    ep_name = st.session_state.get("episode_name", "EP001")
    st.markdown(f"### 🤖 [{ep_name}] 젬마 리서치 참모")
    ...
    # 2. 하단 입력 바 및 기능
    st.markdown("<br>", unsafe_allow_html=True)
    c_plus, c_input, c_model, c_send = st.columns([1, 6, 2, 1.5])
    ...
    with c_plus:
        with st.popover("＋", use_container_width=True):
            st.markdown("**기능 메뉴**")
            if st.button("📁 파일 탐색기 / 로컬 자료 불러오기", use_container_width=True):
                st.info("다음 단계에서 파일 탐색기 연동 예정입니다.")
            if st.button("☁️ Google Drive / Docs 자료 가져오기", use_container_width=True):
            ...
            if st.button("🎬 성공 영상 찾기", use_container_width=True):
                st.info("다음 단계 연동 예정입니다.")
            st.markdown("---")
            if st.button("🧩 마지막 응답을 현재 Part로 Push", use_container_width=True):
            ...
            if st.button("📋 마지막 응답 복사", use_container_width=True):
                last_assistant_msg = None
                for msg in reversed(st.session_state.right_popup_history):
                    if msg.get("role") == "assistant" and msg.get("source") != "사용자 입력":
                        last_assistant_msg = msg
    ...
    with c_model:
        models = ["gemma4:e2b", "gemma4:e4b", "gemini-2.5-flash", "gemini-3-flash", "Tavily 웹 검색"]
        selected_mode = st.selectbox("엔진", models, key="right_gemma_model_select", label_visibility="collapsed")
