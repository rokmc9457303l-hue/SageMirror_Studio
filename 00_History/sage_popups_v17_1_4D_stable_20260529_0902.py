...
def _call_gemma_fast_a_mode(prompt: str, system: str = "", model: str = "gemma4:e2b") -> str:
    """
    [A모드 전용 v17.1.4-D] 빠른 대화 초고속 반환.
    - /api/generate 엔드포인트 사용 (st.sidebar 호출 없음)
    - stream=True 기반 스트리밍 수신 → HTTP 타임아웃 원천 차단
    - thinking 폴백 로직 제거 (빈 응답 시 오류 메시지 직접 반환)
    - keep_alive, num_predict, temperature, top_p 세션 스테이트에서 로드
    """
    try:
        import requests as _req
        import json as _json
        ka_val = st.session_state.get("popup_keep_alive", "10m")
        np_val = st.session_state.get("popup_num_predict", 150)
        temp_val = st.session_state.get("popup_temperature", 0.2)
        tp_val = st.session_state.get("popup_top_p", 0.8)

        # system + user 합성 → /api/generate 단일 prompt 필드로 전달
        full_prompt = ""
        ...
        content = "".join(collected_tokens).strip()
        if not content:
            return "[A모드 Fast 호출 오류] 모델이 빈 응답을 반환했습니다."
        return content
    except Exception as e:
        return f"[A모드 Fast 호출 오류] {e}"
...
def popup_assistant():
    ...
    with tab_chat:
        ...
        if "popup_num_predict" not in st.session_state:
            st.session_state["popup_num_predict"] = 150
        ...
        with st.expander("⚙️ A모드 속도 최적화 매개변수 설정", expanded=False):
            ...
            with c_opt1:
                ...
                np_idx = np_opts.index(cur_np) if cur_np in np_opts else 0
                np_val = st.selectbox("num_predict (출력 최대 토큰 수)", np_opts, index=np_idx, key="p_opt_np_select")
            with c_opt2:
        ...
        if st.session_state.get("popup_last_elapsed"):
            ...
            st.markdown(
                ...
                f"padding:10px 14px;border-radius:6px;font-size:0.85rem;margin-bottom:10px;color:#f5e9d3;'>"
                f"⏱️ <b>A모드 성능 진단:</b><br>"
                f"- <b>모델</b>: <code>{el.get('model')}</code><br>"
                f"- <b>keep_alive</b>: <code>{el.get('keep_alive', '10m')}</code> (캐시 유지)<br>"
                f"- <b>num_predict</b>: <code>{el.get('num_predict', 40)}</code> (제한 수)<br>"
                f"- <b>temperature</b>: <code>{el.get('temperature', 0.2)}</code><br>"
                f"- <b>top_p</b>: <code>{el.get('top_p', 0.8)}</code><br>"
                f"- <b>sys_ctx 글자 수</b>: {el.get('sys_ctx_chars', 0)}자<br>"
                f"- <b>user_prompt 글자 수</b>: {el.get('user_prompt_chars', 0)}자<br>"
                f"- <b>popup_history 복원</b>: {el.get('restore_time', 0.0):.4f}초<br>"
                f"- <b>sys_ctx 생성</b>: {el.get('sys_ctx_time', 0.0):.4f}초<br>"
                f"- <b>fast_api_call</b>: <span style='color:#d4af6a;font-weight:700;'>{el.get('total', 0.0):.2f}초</span><br>"
                f"- <b>JSON 저장</b>: <span style='color:#10B981;font-weight:700;'>{el.get('save', 0.0):.4f}초</span><br>"
                f"- <b>전체</b>: <b>{el.get('overall_time', 0.0):.2f}초</b><br>"
                f"- <b>run_agent_loop</b>: {el.get('run_agent_loop', 'NO')}<br>"
                f"- <b>Tavily</b>: {el.get('tavily', 'NO')}<br>"
                f"- <b>Gemini</b>: {el.get('gemini', 'NO')}<br>"
                f"- <b>Obsidian RAG</b>: {el.get('obsidian_rag', 'NO')}<br>"
                f"- <b>References Memory</b>: {el.get('references_memory', 'NO')}<br>"
                f"- <b>Recent Activity</b>: {el.get('recent_activity', 'NO')}<br>"
                f"- <b>popup_search_history 주입</b>: {el.get('search_history_injected', 'NO')}"
                f"</div>",
                unsafe_allow_html=True,
            )
        ...
        with chat_container:
            ...
            if st.session_state.get("pending_stream"):
                ...
                # 성능 진단용 trace 플래그 초기화
                st.session_state["trace_A_mode_direct_gemma_called"] = "NO"
                st.session_state["trace_run_agent_loop_called"] = "NO"
                st.session_state["trace_tavily"] = "NO"
                st.session_state["trace_gemini"] = "NO"
                st.session_state["trace_obsidian_rag"] = "NO"
                st.session_state["trace_references_memory"] = "NO"
                st.session_state["trace_recent_activity"] = "NO"
                st.session_state["trace_popup_search_history"] = "NO"

                # ── 공통 시스템 컨텍스트 (A/B 모드 공통) ──
                import time
                t_sys_start = time.perf_counter()

                # A 모드: 초경량 시스템 프롬프트 (200자 이내 — 입력 토큰 최소화)
                if current_mode == "A":
                    sys_ctx = (
                        f"[현자의 거울 AI | 파트: {current_part_name}]\n"
                        "역할: 영상 제작 보조 AI.\n"
                        "규칙: 1~3문장 이내 간결 답변. 생각 과정 출력 금지. 즉시 최종 답변만 출력."
                    )
                else:
                    # B 모드: 풍부한 컨텍스트 주입
                    sys_ctx = SAGE_PERSONA + "\n\n"
                ...
                t_sys_end = time.perf_counter()
                sys_ctx_time = t_sys_end - t_sys_start
                sys_ctx_chars = len(sys_ctx)
                user_prompt_chars = len(q_stream)
                ...
                if current_mode == "A":
                    ...
                    with st.spinner(f"💬 {current_model} 응답 생성 중..."):
                        try:
                            ...
                            full_response = _call_gemma_fast_a_mode(
                                q_stream,
                                system=sys_ctx,
                                model=current_model
                            )
                            ...
                            full_response = f"[A모드 Fast 호출 오류] {e}"
                            elapsed_total = 0.0
                    ...
                    try:
                        ...
                        if recent_activity_ctx.strip():
                            st.session_state["trace_recent_activity"] = "YES"
                            sys_ctx += "\n" + recent_activity_ctx + "\n\n"
                    ...
                    if st.session_state.get("popup_use_rag", False):
                        try:
                            ...
                            if ref_items:
                                st.session_state["trace_references_memory"] = "YES"
                        ...
                        if tavily_ctx:
                            ...
                            st.session_state["trace_popup_search_history"] = "YES"
                    ...
                    with st.status("🔮 젬마 에이전트 작동 중...", expanded=True) as status_widget:
                        ...
                        try:
                            st.session_state["trace_run_agent_loop_called"] = "YES"
                ...
                if current_mode == "A":
                    overall_time = sys_ctx_time + elapsed_total + elapsed_save
                    st.session_state.popup_last_elapsed = {
                        "model": current_model,
                        "sys_ctx_chars": sys_ctx_chars,
                        "user_prompt_chars": user_prompt_chars,
                        "restore_time": st.session_state.get("popup_history_restore_time", 0.0),
                        "sys_ctx_time": sys_ctx_time,
                        "total": elapsed_total,
                        "save": elapsed_save,
                        "overall_time": overall_time,
                        "run_agent_loop": st.session_state.get("trace_run_agent_loop_called", "NO"),
                        "tavily": st.session_state.get("trace_tavily", "NO"),
                        "gemini": st.session_state.get("trace_gemini", "NO"),
                        "obsidian_rag": st.session_state.get("trace_obsidian_rag", "NO"),
                        "references_memory": st.session_state.get("trace_references_memory", "NO"),
                        "recent_activity": st.session_state.get("trace_recent_activity", "NO"),
                        "search_history_injected": st.session_state.get("trace_popup_search_history", "NO"),
                        "num_predict": st.session_state.get("popup_num_predict", 40),
