...
🪞 현자의 거울 스튜디오 — Master App v15.9.14

[v15.9.14 업데이트 사항: 2026-05-23]
    ...
    for loop_idx in range(max_loops):
        ...
        if not search_res and tavily_api_key:
            ...
            try:
                ...
                search_res = tavily_search(keyword, tavily_api_key, max_results=5)
            except Exception as e:
                st.error(f"❌ Tavily 검색 중 오류 발생: {e}")
        ...
        research_context = (
            ...
            f"반드시 인용된 웹 정보의 출처를 `[SOURCE: 웹 — {first_url} — 검색일: {today_str}]` 와 같이 구체적인 URL과 함께 명시하라.\n"
            ...
            try:
                ...
                if is_insufficient:
                    ...
                    if not web_search_text and tavily_api_key:
                        ...
                        try:
                            from sage_engine import tavily_search
                            search_res = tavily_search(query_val, tavily_api_key, max_results=5)
                            if "error" not in search_res:
                                web_results = search_res.get("results", [])
                                formatted_tavily = []
...
[요약 및 검수]:"""
                    ...
                    if need_kw:
                        ...
                        if not web_search_text and tavily_api_key:
                            try:
                                from sage_engine import tavily_search
                                search_res = tavily_search(need_kw, tavily_api_key, max_results=5)
                                if "error" not in search_res:
                                    web_results = search_res.get("results", [])
                                    formatted_tavily = []
                                    for idx, r in enumerate(web_results, 1):
                                        formatted_tavily.append(f"[{idx}] {r.get('title')}: {r.get('url')}\n내용: {r.get('content')}")
                                    web_search_text = "\n\n".join(formatted_tavily)
                            except Exception:
                                pass
...
[요약 및 검수]:"""
                ...
                md_content = f"# [[Part {part_key[-1]} RAG 검색 — {query_val}]]\n\n"
                ...
                if obs_results:
                    for idx, r in enumerate(obs_results, 1):
                        ...
                        md_content += f"  - **출처**: [SOURCE: Obsidian — {r.get('path')}]\n"
                else:
                    md_content += "- 검색 결과가 존재하지 않습니다.\n"
                md_content += "\n"
                
                md_content += "## 🌐 보완 리서치 결과\n"
                if web_search_text:
                    md_content += f"{final_summary}\n"
                else:
                    md_content += "- 추가적인 보완 리서치가 진행되지 않았거나 불필요하여 기록되지 않았습니다.\n"
                md_content += "\n"
                
                md_content += "## 🔗 연결 개념\n"
                for t in [tag.strip() for tag in query_val.split(",") if tag.strip()][:5]:
                    md_content += f"- [[{t}]]\n"
                for t in ["고독", "후회", "상실", "쇼펜하우어", "성경"]:
                    if t not in query_val:
                        md_content += f"- [[{t}]]\n"
                md_content += "\n"
                
                md_content += "## 📚 출처\n"
                if obs_results:
                    for r in obs_results[:2]:
                        md_content += f"[SOURCE: Obsidian — {r.get('path')}]\n"
                if web_results:
                    for r in web_results[:2]:
                        md_content += f"[SOURCE: 웹 — {r.get('url')} — 검색일: {today_str}]\n"
                if not obs_results and not web_results:
                    md_content += f"[SOURCE: 출처 미확인 — {selected_model} 생성]\n"
                md_content += "\n"
                
                md_content += f"## 다음 파트 전달 메모\n이 검색 결과는 {part_label} 생성 프롬프트의 RAG Context로 사용됨.\n\n"
                ...
                if obs_file:
                    lock_file_readonly(obs_file)
                    st.toast(f"💾 옵시디언 자동 저장 성공: Part{part_key[-1]}_RAG_{query_val}", icon="💾")
...
st.set_page_config(
    ...
    page_title="Sage's Mirror Studio v15.9.9",
...
def save_workspace_state():
    ...
    keys_to_save = [
        ...
        "path_obsidian", "github_repo_url", "github_pat", "tavily_api_key", "youtube_api_key", "gemini_api_key", "p1_gemini_model", "p1_channel_top10", "p1_benchmark_channel", "p1_search_keywords",
...
- 내용 없이 구조만 채우기""",
        ...
        "p2_bench_prompt": """[작업 지시 — 벤치마킹 & 주제 추출]
...
[SOURCE: 옵시디언 RAG / 댓글 분석 / Gemma 추론]""",
        ...
        "p2_research_prompt": """[작업 지시 — 자료조사 & 지식 융합]

너는 현자의 거울 스튜디오의 수석 연금술사(Alchemist)이다.
선정된 주제를 바탕으로 철학·심리학·성경·에세이를 융합한 자료조사 초안을 작성하라.
...
① TopicMemory → 동일/유사 주제 기존 자료 확인
② ResearchMemory → 관련 철학·심리·성경 자료 확인
③ 태그 검색: [[고독]] [[후회]] [[상실]] [[관계]] [[용서]] [[쇼펜하우어]] [[칼 융]] [[빅터 프랭클]] [[몽테뉴]] [[에세이]] [[다크심리학]] [[시편]] [[잠언]] [[전도서]] [[욥기]] [[성경]]
✅ RAG 있으면: [[위키링크]] + [SOURCE: 옵시디언 — 파일명]
❌ RAG 없으면: [NEED_RESEARCH: 키워드] 후 STEP 2

──────────────────────────────────────
[STEP 2] 지식 융합 순서
──────────────────────────────────────
1순위 — 성경 전체 (시편·잠언·전도서·욥기·이사야·신약)
- 반드시 실제 구절 확인 후 인용
- [SOURCE: 성경 — 책명 NN:NN] / 존재하지 않는 구절 절대 금지

2순위 — 철학 (감정 상태에 따라 선택)
- 의미상실 → 빅터 프랭클 / 고독·허무 → 쇼펜하우어
- 내면 그림자 → 칼 융 / 절제·현재 → 스토아 (마르쿠스 아우렐리우스)
- [SOURCE: 철학 — 저자명, 저서명]
...
4순위 — 다크심리학 (반드시 리서치 기반)
- [NEED_RESEARCH: 다크심리학 — 키워드] 후 사용
- [SOURCE: 다크심리학 — 출처명]

5순위 — 웹 리서치 (RAG 부족시만)
- [NEED_RESEARCH: 키워드] 후 [SOURCE: 웹 — 출처명]

──────────────────────────────────────
[출력 필수 항목]
──────────────────────────────────────
1. 핵심 제목 / 2. 핵심 키워드([[위키링크]])
3. 시청자 감정 고통 요약 (댓글 기반)
4. 성경 통찰 [SOURCE] / 5. 철학 통찰 [SOURCE]
6. 에세이 통찰 [SOURCE] / 7. 다크심리학 통찰 [SOURCE]
8. Part 2 전달 메모

──────────────────────────────────────
[절대 금지]
──────────────────────────────────────
- 가짜 성경 구절 / 출처 없는 인용 / RAG 없이 상상 생성""",
        ...
        "p2_plan_prompt": """[작업 지시 — Part 3-4 전달 패킷 설계]

너는 현자의 거울 스튜디오의 수석 연금술사(Alchemist)이다.
자료조사 결과를 바탕으로 Part 3-4 (Architect & Writer)가
바로 대본 작업에 착수할 수 있도록 완결된 전달 패킷을 설계하라.
내레이터는 @Protagonist — 60대 중후반 서양 철학 체화한 현자다.
...
9. 💬 오늘의 명언 (출처 명기 필수)
10. 📦 Part 3-4 인계 메모
    (Architect & Writer가 반드시 알아야 할 핵심)
...
with st.sidebar:
    ...
    st.markdown(f"### {APP_TITLE} **v15.9.8**")
...
def render_top_panel():
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 마스터 프롬프트", expanded=False):
        ...
        with L:
            ...
            # 🔍 즉석 실시간 구글 리서치 & 저장 UI
            st.markdown("<div style='margin-top: 10px; margin-bottom: 5px; font-size: 0.85em; color: #d4af6a; font-weight: bold;'>🔍 즉석 실시간 구글 리서치 & 저장</div>", unsafe_allow_html=True)

            research_cols = st.columns([3.0, 1.2, 0.4])
            ...
            with research_cols[0]:
                ...
                search_query = st.text_input(
                    ...
                    placeholder="검색 키워드 입력 (예: 쇼펜하우어 고독의 의미)",

                    key="top_instant_search_query",
            ...
            with research_cols[1]:
                ...
                if st.button("💾 리서치 & 저장", key="top_instant_search_btn", use_container_width=True):
                    ...
                    if not search_query.strip():
                        ...
                        if not gemini_api_key:
                            ...
                            st.error("🤖 Gemini API Key가 설정되지 않았습니다. 사이드바 설정을 확인해 주세요.")

                        else:

                            with st.spinner("🔍 제미나이가 구글 실시간 검색 및 옵시디언 저장 중..."):

                                gemini_model_name = st.session_state.get("p1_gemini_model", "gemini-2.5-flash")

                                search_res = gemini_search_grounding(search_query, gemini_api_key, model_name=gemini_model_name)

                                if "error" in search_res:

                                    st.error(f"❌ 검색 오류: {search_res['error']}")

                                else:

                                    results = search_res.get("results", [])

                                    research_content = f"### 🔍 수동 리서치 검색어: {search_query}\n\n"
                                    ...
                                    obs_file = save_obsidian_memory(
                                        ...
                                        title=f"Manual_Research_{search_query}",

                                        content=research_content,

                                        source=f"수동 실시간 검색 — {search_query}"
                                    ...
                                    if obs_file:
                                        ...
                                        st.toast(f"💾 리서치 성공 및 옵시디언 즉시 저장 완료!", icon="💾")

                                        if GIT_AVAILABLE:

                                            try:

                                                success, msg = auto_git_push(f"Manual Save Research: {search_query}")
            ...
            with research_cols[2]:
                ...
                is_saved = st.session_state.get("research_auto_save_success", False)

                led_color = "#60a5fa" if is_saved else "#e2e8f0"

                led_tooltip = "리서치 자동저장 완료" if is_saved else "리서치 자동저장 대기 중"
            ...
            btn_cols = st.columns([1.2, 1.4, 1.4])

            with btn_cols[0]:

                if st.button("[SEARCH] 편집", key="ob_btn", use_container_width=True):
            ...
            with btn_cols[1]:
                ...
                btn_label = "💾 자동저장: ON" if auto_save_active else "💾 자동저장: OFF"

                if st.button(btn_label, key="ob_auto_save_btn", use_container_width=True):

                    st.session_state.research_auto_save = not auto_save_active
            ...
            with btn_cols[2]:
                ...
                with st.popover("📂 저장 확인", use_container_width=True):

                    st.caption("📂 최근 저장된 리서치 목록 (최대 8개)")
...
def render_part1():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p1_popup_btn"):
...
"""
    ...
    DEFAULT_OBS_TAGS = (
        "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 외로움, 죽음, 인간관계, "
        "쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 마르쿠스 아우렐리우스, "
        "몽테뉴, 에세이, 다크심리학, "
        "시편, 잠언, 전도서, 욥기, 이사야, 성경"
    )
    obsidian_query = st.text_input(
        ...
        value=st.session_state.get("p1_obsidian_search_query_val", DEFAULT_OBS_TAGS),
    ...
    st.caption("💡 기본 태그: 고독·후회·상실·쇼펜하우어·칼 융·빅터 프랭클·몽테뉴·에세이·다크심리학·시편·잠언 등 — 직접 수정 가능")
    ...
    with tab_research:
        ...
        with st.container(border=True):
            ...
            # Part 1 RAG 검색 UI 주입 (접힌 상태로 제공하여 기존 UI 보호)
...
def render_part2():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p2_popup_btn"):
...
"""
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P2_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part2", 
            "Part 2: 총괄기획", 
            DEFAULT_P2_TAGS, 
...
def render_part5():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p5_popup_btn"):
...
def render_part5_image():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p5img_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P4_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part4", 
            "Part 4: 이미지 생성용 프롬프트 생성", 
            DEFAULT_P4_TAGS, 
...
def render_part6_video():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p6_vid_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P5_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part5", 
            "Part 5: 비디오 클립 매칭 및 나레이션 매핑", 
            DEFAULT_P5_TAGS, 
    ...
    # ── Obsidian 및 Git Push 통합 저장 버튼 (RULE 4-3 표준) ──

    if st.button("💾 [파트 5: 영상 생성] 옵시디언 자동 백업", type="primary", use_container_width=True, key="p6_video_obsidian_backup_btn"):
...
def render_part34():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p34_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P3_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part3", 
            "Part 3: 대본 작성", 
            DEFAULT_P3_TAGS, 
...
def render_part6_opal():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p6_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P6_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part6", 
            "Part 6: 나레이션 & 배경음악", 
            DEFAULT_P6_TAGS, 
        ...
        if st.session_state.get("path_obsidian"):
            ...
            try:
                ...
                if st.session_state.get("p6_opal_df") is not None:
                    ...
                    save_to_outputs_dir("04_Narration", "narration_dispatch.csv", is_csv=True, df=st.session_state.p6_opal_df)

                    bgm_content = f"BGM 분위기 프리셋: {st.session_state.get('p6_bgm_selection', '비장한 성경 낭독풍')}\n나레이션 볼륨 비율: {st.session_state.get('p6_mixing_ratio', 80)}%\n"

                    save_to_outputs_dir("05_BGM", "bgm_settings.txt", content=bgm_content)

                    st.session_state.p6_outputs_saved = True
...
def render_part7_capcut():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p7_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P7_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part7", 
            "Part 7: 숏폼 생성", 
            DEFAULT_P7_TAGS, 
    ...
    if st.session_state.get("p7_shortform_hook"):
        st.session_state.p7_shortform_hook = st.text_area(
            "생성된 숏폼 후킹 및 핵심 메시지 (수정 가능)",
            value=st.session_state.p7_shortform_hook,
            height=200,
            key="p7_hook_result_area",
            disabled=is_locked
        )
    ...
    if st.button("💾 파트 7 옵시디언 자동 백업", type="primary", use_container_width=True, key="p7_backup_btn", disabled=is_locked):
        ...
        if st.session_state.get("path_obsidian"):
            ...
            if st.session_state.get("p7_shortform_hook"):
                md += "## 📣 숏폼 후킹 및 핵심 메시\n"
                md += st.session_state.p7_shortform_hook + "\n\n"
...
def render_part8_dashboard():
    ...
    with c_control:
        ...
        with c_pop_col:
            ...
            if st.button("🤖 젬마 어시스턴트", type="secondary", use_container_width=True, key="p8_popup_btn"):
    ...
    with st.expander("🔍 RAG 보완 검색 설정 (접기/펼치기)", expanded=False):
        DEFAULT_P8_TAGS = "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, 쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, 에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        render_obsidian_rag_search(
            "part8", 
            "Part 8: 캡컷 최종 조립", 
            DEFAULT_P8_TAGS, 
    ...
    if st.session_state.get("p8_production_guide"):
        st.session_state.p8_production_guide = st.text_area(
            "생성된 최종 검수 가이드라인 (수정 가능)",
            value=st.session_state.p8_production_guide,
            height=200,
            key="p8_guide_result_area",
            disabled=is_locked
        )
    ...
    if st.button("💾 파트 8 최종 생산 세션 백업", type="primary", use_container_width=True, key="p8_backup_btn", disabled=is_locked):
        ...
        if st.session_state.get("path_obsidian"):
            ...
            if st.session_state.get("p8_production_guide"):
                md += "## 📋 AI 최종 검수 가이드라인\n"
                md += st.session_state.p8_production_guide + "\n\n"
