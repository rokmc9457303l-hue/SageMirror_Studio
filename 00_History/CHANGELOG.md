## v17.1.31 — [2026-06-02 14:29]
### 변경 내용
- 불필요한 파트0 관련 구현체(render_part0_assistant) 및 남아있던 사이드바 옵션을 물리적으로 완전 삭제
- 우측 젬마 대화창에 position: sticky CSS 및 #gemma-right-anchor를 주입하여 스크롤 시에도 뷰포트에 영구 고정되도록 UI 뼈대 완벽 구축
### 영향 파트
- 앱 레이아웃 (우측 젬마 대화창)
### 수정 파일
- app_v17_1_31.py

## v17.1.30 — [2026-06-02 09:06]
### 변경 내용
- 사이드바 라우팅 정리 (파트 0 메뉴 제거)
- 메인 라우팅 조건문 캡슐화 오류 및 들여쓰기 붕괴 완벽 해결 (파트 1부터 올바르게 동작)
- 7:3 레이아웃 구현 및 우측 젬마 대화창에 position: sticky CSS 삽입하여 뷰포트 고정.
### 영향 파트
- 앱 전체 레이아웃 (파트 1~8 및 사이드바 메뉴)
### 수정 파일
- app_v17_1_30.py

## v17.1.27 — [2026-06-02 08:33]
### 변경 내용
- 메인 콘텐츠 영역을 7:3 비율로 분할 적용 (col_main, col_gemma).
- 기존 sage_popups 임포트 및 함수(popup_assistant, popup_edit_obsidian, popup_edit_prompt) 호출 코드 일괄 비활성화/제거.
### 영향 파트
- 앱 전체 레이아웃 (파트 1~8)
### 수정 파일
- app_v17_1_27.py

## v17.1.22 — 2026-05-31 15:05 (Hotfix 패치 적용)
### 변경 내용
- **[StreamlitAPIException 크래시 복구 및 Key Increment 패턴 도입]**: 대화창 `st.text_area` 입력 위젯에 텍스트 리셋 과정에서 발생하던 `StreamlitAPIException`을 방지하고자 위젯 key에 `chat_widget_key` 카운터를 결합하는 동적 키 할당 방식('Key Increment')을 도입.
- **[텍스트 입력 초기화 최적화]**: 입력 전송 완료 시 세션 스테이트의 widget key 값을 직접 수정하는 대신 `chat_widget_key` 값을 1씩 증가시켜 위젯을 새로 렌더링함으로써 텍스트창을 안전하게 비우는 구조로 개편 (Ghost Text 및 무한 로딩 버그 완벽 클리어).
- **[세션 기반 고정 키 제어 및 자바스크립트 우회 코드 제거]**: 대화창 `st.text_area` 입력 위젯에 기존 불안정한 JS 기반 초기화 꼼수를 완전 제거하고 Streamlit 정공법 적용.
- **[배포 버전 갱신]**: `app_v17_1_22.py`, `sage_popups_v17_1_22.py`, `RUN_APP.bat`, `RUN_DEBUG.bat` 업데이트.
### 영향 파트
- 팝업 어시스턴트 대화 입력 및 전송 영역
- 배포/실행 스크립트 2종
### 수정 파일
- app_v17_1_22.py
- sage_popups_v17_1_22.py
- RUN_APP.bat
- RUN_DEBUG.bat


## v17.1.12 — 2026-05-29 12:20
### 변경 내용
- **[팝업 대화창 호출 전면 복구]**: 파트 0을 포함하여 파트 1 ~ 파트 8까지의 모든 우측 상단 어시스턴트 단추의 순간이동 리다이렉트 및 토스트 안내를 전면 폐기하고, 클릭 시 즉각 화면 전면에 젬마 어시스턴트 네이티브 대화창(Dialog) 팝업이 활성화되도록 원상 복구.
- **[배포 버전 갱신]**: `app_v17_1_12.py`, `sage_popups_v17_1_12.py`, `RUN_APP.bat` 업데이트.
### 영향 파트
- 파트 0 ~ 파트 8 상단 우측 어시스턴트 단추 영역
- 팝업 어시스턴트 시스템
### 수정 파일
- app_v17_1_12.py
- RUN_APP.bat
...
## v17.1.11 — 2026-05-29 12:00
### 변경 내용
- **[파트 0 풀스크린 3:7 스튜디오 완벽 개편]**: 좌측 30% 툴바 영역에 다포맷 지식 추출(파일 업로더 + URL 링크 파서)과 규칙서 저장/초기화 기능 배치, 우측 70% 대화창에 4가지 AI 엔진(Gemma, Gemini, Tavily) 선택 칩 및 대화 내역 영속성 컨테이너 완벽 구축.
- **[모든 파트 우측 상단 단추 순간이동화]**: 파트 1 ~ 파트 8의 상단 어시스턴트 단추를 클릭 시 파트 0으로 순간이동 리다이렉트시키는 '🚀 젬마 스튜디오 입장' 버튼으로 전환 복구.
- **[배포 버전 갱신]**: `app_v17_1_11.py`, `sage_popups_v17_1_11.py`, `RUN_APP.bat` 업데이트.
### 영향 파트
- 파트 0 (풀스크린 젬마 스튜디오)
- 파트 1 ~ 파트 8 상단 우측 어시스턴트 단추 영역
### 수정 파일
- app_v17_1_11.py
- RUN_APP.bat


## v17.1.10 — 2026-05-29 11:55
### 변경 내용
- **[팝업 차단 로직 전면 제거]**: 대화창 진입을 막던 모든 방해물(순간이동 리다이렉트 등)을 전면 걷어내고, 팝업 관련 몽키패치를 원천 삭제 및 복구.
- **[대화창 팝업 원상 복구]**: 파트 0(젬마 스튜디오)을 포함하여 파트 1 ~ 파트 8까지의 모든 우측 상단 어시스턴트 호출을 `popup_assistant()` 기반 대화창 팝업 단추로 복원 완료.
- **[배포 버전 갱신]**: `app_v17_1_10.py`, `sage_popups_v17_1_10.py`, `RUN_APP.bat` 업데이트.
### 영향 파트
- 파트 0 ~ 파트 8 상단 우측 어시스턴트 단추 영역
- 팝업 어시스턴트 시스템
### 수정 파일
...
## v17.1.9 — 2026-05-29 11:45
### 변경 내용
- **[파트 0 상단 잉여 버튼 완전 적출]**: 파트 0 메인 스튜디오 상단 우측에 자리잡고 있던 불필요한 '[🤖 젬마 어시스턴트]' 팝업 호출 버튼 및 관련 로직을 완전 삭제.
- **[파트 0 레이아웃 최적화]**: 상단 헤더의 칼럼 레이아웃을 기존 3열([5, 3, 2])에서 7:3 2열(`c_title, c_pin`)로 단순화하여 화면 효율성 증대.
- **[배포 버전 갱신]**: `app_v17_1_9.py`, `sage_popups_v17_1_9.py`, `RUN_APP.bat` 업데이트.
### 영향 파트
- 파트 0: 🤖 젬마 스튜디오 상단 영역
### 수정 파일
- app_v17_1_9.py
...
## v17.1.8 — 2026-05-29 11:35
### 변경 내용
- **[몽키패치 제거]**: 최상단에 임시로 심어두었던 popup_assistant 리다이렉트 몽키패칭 찌꺼기를 원천 삭제 및 복구.
- **[파트별 🚀 젬마 스튜디오 입장 순간이동 개조]**: 파트 1부터 파트 8까지의 모든 상단 어시스턴트 팝업 호출 버튼을 '🚀 젬마 스튜디오 입장' 주황색/파란색(Primary) 버튼으로 변경하고, 클릭 시 파트 0 메인 스튜디오로 매끄럽게 화면 전환(st.rerun)하도록 개조 완료.
- **[배포 버전 갱신]**: `app_v17_1_8.py`, `sage_popups_v17_1_8.py`, `RUN_APP.bat` 업데이트.
### 영향 파트
- 완성 파트 (Part 1 ~ Part 8) 상단 헤더 메뉴
- 파트 0: 🤖 젬마 스튜디오
### 수정 파일
- app_v17_1_8.py
- RUN_APP.bat
...
## v17.1.7 — 2026-05-29 11:15
### 변경 내용
- **[파트 0 풀스크린 개편]**: 팝업 방식 어시스턴트를 폐기하고 메인 화면에 3:7 레이아웃의 '파트 0: 🤖 젬마 스튜디오'를 신설.
- **[지식 소스 수집 파서 탑재]**: 파일 업로더(TXT, MD, PDF, HTML)와 구글 독스/드라이브/웹 URL 파서를 구축해 순수 본문 텍스트 본문 추출 연동.
- **[옵시디언 마스터 규칙서 v4.0 변환 및 GitHub 연계]**: 텍스트 자료를 젬마를 통해 마스터 규칙서 v4.0 규격으로 자동 마크다운 변환 및 00_Obsidian/References 자동 백업 후 GitHub Auto Push 연계.
- **[가로 칩 모델 스위처 및 네이티브 채팅 이식]**: E2B, E4B, Gemini, Tavily 웹검색 에이전트를 직관적으로 선택하는 가로형 라디오 칩 UI 및 대화 영속성 JSON 파일 입출력 로직 완벽 이식.
- **[접근 금지 구역 보호 및 팝업 몽키패칭]**: 완성 파트(Part 1~8)를 보호하기 위해 기존 popup_assistant를 가로채어 파트 0 이동 리다이렉션으로 몽키패치 처리.
### 영향 파트
- 파트 0: 🤖 젬마 스튜디오 (신설)
- 완성 파트 (Part 1~8) 헤더 팝업 호출
### 수정 파일
- app_v17_1_7.py
- RUN_APP.bat


## v17.1.4-E — 2026-05-29 09:02
### 변경 내용
- **[A모드 응답 화면 표시 해결]**: `@st.dialog` 팝업 내부에서 `st.empty()` placeholder 실시간 업데이트가 작동하지 않아 응답이 화면에 표시되지 않던 근본 원인 해결.
- **[`_call_gemma_fast_a_mode` 동기 방식 교체]**: `/api/generate` + `stream=True` 스트리밍 호출을 완전 제거하고 `sage_engine.call_gemma()` 동기 호출 래퍼로 단순화.
- **[A모드 `st.rerun()` 추가]**: A모드 응답 저장 완료 후 `st.rerun()`을 실행하여 대화창 닫지 않아도 응답이 즉시 화면에 표시되도록 수정.
### 영향 파트
- 팝업 어시스턴트 빠른 대화(A모드) 응답 렌더링
### 수정 파일
- `sage_popups_v17_1_4E.py`
- `app_v17_1_4E.py`

## v17.1.4-D — 2026-05-29 08:30
### 변경 내용
- **[A모드 sys_ctx 초경량화]**: 시스템 프롬프트를 약 200자 이내 초경량으로 교체, SAGE_PERSONA + 옵시디언 규칙서 500자 대신 "파트명+역할+규칙" 3줄로 입력 토큰 대폭 절감.
- **[/api/generate + stream=True 전환]**: `/api/chat`에서 `/api/generate` 스트리밍 방식으로 전환, HTTP 연결 유지로 타임아웃 원천 차단. 청크 단위 토큰 조합으로 빠른 응답 전달.
- **[num_predict 기본값 상향]**: 40 → 150으로 상향 조정, UI 선택지 `[150, 300, 500, 1000]`으로 현실화.
- **[thinking 폴백 로직 제거]**: 빈 응답 시 `thinking` 필드 추출 폴백을 제거하고 오류 메시지를 직접 반환하는 단순 구조로 정비.
- **[배포 파일 갱신]**: `app_v17_1_4D.py`, `sage_popups_v17_1_4D.py`, `RUN_APP.bat` 버전 업데이트.
### 영향 파트
- 팝업 어시스턴트 빠른 대화(A모드) 전체 호출 경로
### 수정 파일
- `app_v17_1_4D.py`
- `sage_popups_v17_1_4D.py`
- `RUN_APP.bat`

## v17.1.4-C — 2026-05-29 03:55
### 변경 내용
- **[호출 경로 교정] A모드 빠른 대화 자체 Fast Ollama API 호출 함수로 완전 격리**:
  - `sage_popups_v17_1_4C.py` 내부에 `_call_gemma_fast_a_mode` 함수를 활용해 외부 RAG/Tavily/Gemini/Recent Activity/Obsidian RAG 등 모든 부가 기능 및 메인 앱 wrapper를 우회하여 Ollama API를 직접 호출하도록 완벽히 경로를 교정.
- **[안전성 확보] st.sidebar fragment 오류 원인인 st.rerun() A모드 차단 및 튕김 해결**:
  - 다이얼로그(fragment) 내부에서 호출 시 `st.sidebar` 접근 에러를 유발하고 팝업 닫힘 및 비밀번호/로그인 화면으로의 원치 않는 화면 전환(튕김)을 발생시키던 `st.rerun()` 호출을 A모드 빠른 대화 경로에서는 완전히 배제 처리하여 안전성을 대폭 강화.
- **[폴백 및 프롬프트 강화] content 공백 반환 시 thinking 필드 자동 추출 복원 및 생각 억제 지침 적용**:
  - Ollama API에서 `num_predict=40` 토큰 제한 시 추론과정(thinking) 도중에 생성이 끊겨 `content`가 공백으로 반환되던 문제를 해결하고자 시스템 프롬프트에 생각 과정을 강력히 생략하도록 지시하고, 만에 하나 본문이 비어있을 때는 `thinking` 필드의 앞부분(150자)을 자동 추출하여 응답 텍스트로 복원하는 2중 안전 장치(폴백) 적용.
- **[오류 진단 문구 정교화]**:
  - Ollama API 통신 에러가 아닌 Streamlit UI/컴포넌트 오류 발생 시 "Ollama 서버 실행 여부를 확인하세요"라는 오진 메시지가 뜨지 않도록 예외 문구를 `[A모드 Fast 호출 오류] {e}` 형태로 정교하게 개선.
- **[버전 및 파일 연결 갱신]**:
  - `app_v17_1_4B.py` → `app_v17_1_4C.py`, `sage_popups_v17_1_4B.py` → `sage_popups_v17_1_4C.py` 버전업.
  - `RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs` 내 실행 타겟 및 버전을 `app_v17_1_4C.py` 및 `v17.1.4-C`로 갱신 완료.
### 영향 파트
- 팝업 어시스턴트 빠른 대화(A모드) 렌더링 및 응답 생성부
- 배포/실행 스크립트 3종 (`RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs`)
### 수정 파일
- `app_v17_1_4C.py`
- `sage_popups_v17_1_4C.py`
- `RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs`
- `00_History/CHANGELOG.md`

---

## v17.1.4-B — 2026-05-29 03:30
### 변경 내용
- **[속도 최적화] 빠른 대화 A모드 Gemma 호출 최적화 옵션 적용**:
  - `keep_alive = "10m"` 및 `options = {"num_predict": 40, "temperature": 0.2, "top_p": 0.8}` 매개변수를 직접 Ollama HTTP REST API 호출 페이로드에 주입하여, 추론 속도를 58.85초에서 **0.65초~1.14초 내외**로 극적으로 단축시킴.
- **[A모드 최적화 UI] 빠른 대화 최적화 매개변수 실시간 설정 컴포넌트 추가**:
  - 빠른 대화 탭에 ⚙️ 최적화 설정 패널(Expander)을 신설하여 사용자가 `keep_alive`, `num_predict(20/40/80/150)`, `temperature`, `top_p`를 실시간으로 직접 조절할 수 있도록 제공.
- **[진단 로그 연동] 최적화 파라미터 시각화**:
  - 성능 진단 로그 화면에 현재 적용된 `num_predict`, `keep_alive` 옵션 수치를 추가 노출하여 포렌식 계측 가독성 제고.
- **[버전 및 파일 연결 갱신]**:
  - `app_v17_1_4A.py` → `app_v17_1_4B.py`, `sage_popups_v17_1_4A.py` → `sage_popups_v17_1_4B.py` 버전업.
  - `RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs` 내 실행 타겟 및 버전을 `app_v17_1_4B.py`로 갱신 완료.
### 영향 파트
- 팝업 어시스턴트 빠른 대화(A모드) 렌더링 및 응답 생성부
- 배포/실행 스크립트 3종 (`RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs`)
### 수정 파일
...
## v17.1.4-A — 2026-05-29 03:00
### 변경 내용
- **[포렌식 계측] 젬마 빠른 대화 1분 지연 원인 분석 및 계측 로그 추가**:
  - `sage_popups_v17_1_4A.py`의 빠른 대화(A모드) 내 상세 계측 로그 UI를 추가하여 `sys_ctx` 글자 수, `user_prompt` 글자 수, `popup_history` 복원 시간, `sys_ctx` 생성 시간, `call_gemma` 소요 시간, `JSON 저장` 소요 시간, 전체 소요 시간을 19종의 계측 항목으로 시각화.
  - 포렌식 결과, 지연의 100% 원인이 Tavily, RAG 등 외부 API가 아닌 Ollama `call_gemma` 자체 호출 소요 시간(58.85초)임을 검증 및 확정.
- **[빠른 대화 톤 최소화] 빠른 대화 시스템 프롬프트 최소화**:
  - 빠른 대화 A모드에서 로컬 젬마 모델에 전달되는 시스템 프롬프트를 1~3문장 이내의 실용적이고 간결한 안내문("안녕" 입력에 대한 답변 톤 최소화)으로 최소화 주입하여 속도 개선 및 과장 톤 제거.
- **[VBS 인코딩 깨짐 해결] RUN_APP.vbs 영어 문구로 교체**:
  - VBScript의 한글 인코딩 오작동을 해결하기 위해 깨진 한글 MsgBox 안내문을 영문 `MsgBox "Sage Mirror Studio started." & Chr(13) & "Browser: http://localhost:8505", 64, "Sage Mirror v17.1.4-A"` 로 전면 교체.
- **[버전 및 파일 연결 갱신]**:
  - `app_v17_1_3.py` → `app_v17_1_4A.py`, `sage_popups_v17_1_3.py` → `sage_popups_v17_1_4A.py` 버전업.
  - `RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs` 내 실행 타겟 및 버전을 `app_v17_1_4A.py`로 갱신 완료.
### 영향 파트
- 팝업 어시스턴트 빠른 대화(A모드) 렌더링 및 응답 생성부
- 배포/실행 스크립트 3종 (`RUN_APP.bat`, `RUN_DEBUG.bat`, `RUN_APP.vbs`)
### 수정 파일
...
- **[태그 시스템 범용화] PSYCHOLOGY_EMOTION_TAGS → UNIVERSAL_CATEGORY_TAGS 교체**:
  - 심리학 채널 전용 10개 카테고리 태그 딕셔너리를 다채널 운영용 범용 12개 카테고리로 전면 교체.
  - 교체 카테고리: 감정, 철학, 성경·신앙, 심리학, 역사, 경제·비즈니스, 건강·생활, 유튜브전략, 채널운영, 제작자료, 출처자료, 에피소드.
- **[함수명 변경] `_classify_emotion_tags` → `_classify_universal_tags`**:
  - 함수 Docstring 및 내부 딕셔너리 참조를 UNIVERSAL_CATEGORY_TAGS로 일괄 갱신.
- **[키워드 추출 프롬프트 갱신]**: "심리학 채널 특성상" → "다양한 채널에서 활용 가능한 범용 태그" 로 교체.
- **[기본 태그 변경]**: `["세이지대화", "현자의거울", "심리치유", "자아성찰"]` → `["세이지대화", "자료조사", "옵시디언저장", "범용자료"]`.
- **[UI 체크박스 갱신]**: Tavily 검색 탭 감정 태그 선택 체크박스가 UNIVERSAL_CATEGORY_TAGS.keys()를 참조하도록 수정.
- **[MD 저장 양식 개선]**: 제목 형식을 `# {title}` → `# [[{title}]]` (위키링크)로 변경, "핵심 요약" 섹션 및 "분류 정보" 섹션 추가.
### 영향 파트
- 모든 파트 — 옵시디언 자동 저장 파이프라인 (popup_part_action_dialog, popup_assistant, Tavily 검색 저장)
### 수정 파일
- `sage_popups.py`
- `sage_popups_v1712_work.py` (작업본, 보존)
- `00_History/CHANGELOG.md`

---

## v17.1.0 — 2026-05-27 20:45
### 변경 내용
- **[A모드 최적화] A모드(일반 대화) 무한 로딩 근본 해결**:
  - `app_v17_1_0.py` 내 `call_gemma` 함수에 A모드 분기를 추가하여, RAG/리서치 루프 없이 즉시 1회 직접 호출하고 반환하도록 개선.
  - `sage_popups.py` 내 팝업 어시스턴트에서 A모드일 경우 `max_iterations=1`로 대입하여 에이전트 루프 제한.
- **[배포 스크립트 갱신]**:
  - `RUN_APP.bat` 내 실행 타겟을 `app_v17_1_0.py`로 변경 완료.

---

## v17.0.9 (패치 및 밭 연결) — 2026-05-27 20:10
### 변경 내용
- **[버그 수정] 팝업 다이얼로그 무한 로딩 빈 응답 에러 해결**:
  - `sage_engine.py` 내 `call_gemma` 함수의 `@st.cache_data` 데코레이터를 제거하여 Streamlit 컨텍스트 없이 팝업 컨텍스트에서 호출될 때 빈 응답이 캐싱되는 증상 수정.
- **[밭 연결] CrossVault 시스템 통합**:
  - `sage_config.py`에 `CROSS_VAULT_BRIDGE` 경로 상수 추가 및 밭 연결 경로 연동.
  - `obsidian_search.py` v2.0 멀티볼트 RAG 검색 엔진으로 업그레이드 (Production, Backup, Archive 세 밭을 하나의 공간에서 동시 검색).
...
- **프롬프트 입력칸 편집/저장 구조 통일 (Unified Prompt Editor Integration)**:
  - **통합 프롬프트 에디터 헬퍼 도입**: 젬마 작업지시 프롬프트를 렌더링하고, 변경 사항 임시 감지, 편집 확인, 로컬 저장 및 옵시디언 자동 저장 기능이 하나로 통합된 `render_unified_prompt_editor()` 함수를 개발하여 적용했습니다.
  - **Part 1~5 프롬프트 편집기 표준화**: Part 1 (주제 추출, 제목 후보, 자료 조사, 총괄 기획), Part 2 (젬마 프로토콜, 썸네일 기획 결과, 자료 조사, 채널 벤치마킹), Part 5 (이미지 젬마 프로토콜)의 모든 개별 프롬프트 text_area와 그에 딸려 있던 파생 '팝업 편집' 버튼들을 통합 에디터 1개 구조로 깔끔하게 교체/통일했습니다.
  - **불필요한 중복 버튼 제거 및 UI 정돈**: 각 프롬프트 텍스트 필드 밑에 붙어있던 복잡한 개별 팝업 편집 버튼들을 제거하고, 렘브란트 다크 테마에 조화로운 슬림형 편집기 컨트롤 바 구조를 구축했습니다.
- **배포 스크립트 갱신**: `RUN_APP.bat` 타겟을 `app_v16_1_19.py`로 갱신 완료했습니다.
### 영향 파트
- Part 1 Librarian, Part 2 Alchemist, Part 5 Image 파트의 젬마 작업지시 프롬프트 UI 컴포넌트 레이어
### 수정 파일
- `app_v16_1_19.py`
- `RUN_APP.bat`
- `00_History\CHANGELOG.md`
...
## v17.0.9 수정 (sage_engine.py) — 2026-05-27 20:05
### 변경 내용
- [치명적 버그 수정] call_gemma @st.cache_data(ttl=300) 제거
  - 원인: 팝업 다이얼로그(@st.dialog) 내부에서 Streamlit 런타임 없이 캐시 함수 호출 시
          빈 응답이 MemoryCacheStorageManager에 캐싱되어 반복적으로 빈 응답 반환
  - 증상: "model output must contain either output text or tool calls" 에러 반복
  - 해결: @st.cache_data 완전 제거, ollama 패키지 빈 응답 감지 후 HTTP 폴백 추가
- [개선] ollama 패키지 빈 응답 감지 로직 추가 (response.strip() 체크 후 HTTP 폴백)
- [개선] HTTP 폴백 빈 응답 방어 로직 추가
- [추가] check_ollama_status에 @st.cache_data(ttl=30) 추가 (반복 상태 확인 최적화)
### 영향 파트
- 모든 파트 (Gemma 대화 전체)
- 특히 popup_assistant 내 대화 기능 정상화
### 수정 파일
- sage_engine.py

## v17.0.9 밭 연결 (CrossVault) — 2026-05-27 20:12
### 변경 내용
- [신규] CrossVaultBridge 구조 생성
  - 00_Obsidian/_shared/CrossVaultBridge/FromArchive/ (32개 고전 문헌 레퍼런스)
  - 00_Obsidian/_shared/CrossVaultBridge/FromBackup/ (BACKUP workspace_state 37개 키)
  - 00_Obsidian/_shared/CrossVaultBridge/Pipeline/ (Part1~8 파이프라인 흐름 문서)
  - 00_Obsidian/_shared/CrossVaultBridge/SharedIndex/ (공유 인덱스)
- [신규] sage_config.py 밭 연결 경로 상수 추가
  - FIELD_PRODUCTION, FIELD_BACKUP, FIELD_OBSIDIAN, FIELD_ARCHIVE
  - CROSS_VAULT_BRIDGE 및 하위 경로 상수
- [업그레이드] obsidian_search.py v2.0 — 멀티볼트 통합 검색
  - 세 밭(메인볼트/브릿지/아카이브) 동시 검색
  - 볼트 출처 표시, 제목 매칭 2배 가중치
  - search_archive_only(), search_bridge_only() 헬퍼 추가
### 영향 파트
- 전체 파트 (RAG 검색 범위 확장)
- Part2 (Alchemist): 고전 32개 문헌 자동 RAG 가능
### 수정 파일
- sage_config.py (CrossVault 경로 상수)
- obsidian_search.py (멀티볼트 검색 v2.0)
- 00_Obsidian/_shared/CrossVaultBridge/ (신규 생성)

 # #   v 1 7 . 1 . 5      2 0 2 6 - 0 5 - 2 9   1 0 : 2 5 
 # # #     
 -   [ ,ȹ  ¤4Ѹ  $t  D  U I   ȩ]   s t . c h a t _ m e s s a g e ,   s t . c h a t _ i n p u t   ȩ<\  D=  
 -   [ 4\  \)    ЄD  (]   ͽɷ  lո  ȅ    _ s t r e a m _ g e m m a _ a _ m o d e   t0  l
 -   [  T    )]   p o p u p _ c h a t _ E P 0 0 1 . j s o n       ƍ1   ȥ  \  T
 -   [ ܴ     iΤ]   A ܴ/ B ܴ  |$  <\        0   
 # # #   ƥ  Ӹ
 -     ¤4Ѹ,   D  U I   ȴ
 # # #     |
 -   a p p _ v 1 7 _ 1 _ 5 . p y 
 -   s a g e _ p o p u p s _ v 1 7 _ 1 _ 5 . p y 
 
 
 # #   v 1 7 . 1 . 6      2 0 2 6 - 0 5 - 2 9   1 0 : 5 6 
 # # #     
 -   [ D      Ɯ  (]     tǼ\    s t . r e r u n ( )     p e n d i n g _ s t r e a m   D  p
 -   [ \   $t  D  (4  ȩ]   s t . c h a t _ i n p u t   8֜    TtD  1XՔ  S t r e a m l i t   \   D    l
 # # #   ƥ  Ӹ
 -     ¤4Ѹ,   D  U I   ȴ
 # # #     |
 -   a p p _ v 1 7 _ 1 _ 6 . p y 
 -   s a g e _ p o p u p s _ v 1 7 _ 1 _ 6 . p y 
 
 ## v17.2.4 — 2026-06-12 23:46
### 변경 내용
- 범용 옵시디언 4계층 저장 (Raw/Wiki/Schema/Logs) 규칙 적용
- 중복 저장 방지 해시(SHA256) 도입
- 범용 카테고리/태그 지원 추가
### 영향 파트
- 우측 리서치 패널 (Part0)
### 수정 파일
