...
def render_right_gemma_panel():
    st.markdown("### 🤖 젬마 리서치 참모")
    
    models = ["⚡ Gemma (빠른 대화)", "🧠 Gemma (심층 분석)", "🌐 Gemini (리서치)", "🔍 Tavily (웹 검색)"]
    selected_mode = st.radio("🤖 AI 엔진", models, horizontal=False, key="right_gemma_engine_select", label_visibility="collapsed")
    ...
    user_input = st.text_area("질문 또는 리서치 내용 입력", key="right_gemma_input_widget", height=100)
    c_btn1, c_btn2 = st.columns([7, 3])
    with c_btn1:
        send_clicked = st.button("전송", key="right_gemma_send_button", use_container_width=True, type="primary")
    with c_btn2:
        clear_clicked = st.button("초기화", key="right_gemma_clear_btn", use_container_width=True)
    ...
    if last_assistant_msg:
        if st.button("마지막 응답을 보강 자료로 밀어넣기 (Push)", key="right_gemma_push_last_response_btn"):
            ...
            st.success("보강 자료가 현재 Part로 전달 대기 상태가 되었습니다.")
        ...
        with chat_container.chat_message("assistant"):
            ...
            if new_model_id == "tavily":
                ...
                with st.spinner("Gemini 리서치 중..."):
                    stream_gen = stream_gemini(user_input, system_prompt=sys_prompt)
                    response = st.write_stream(stream_gen)
                    source_label = "Gemini API"
            else:
