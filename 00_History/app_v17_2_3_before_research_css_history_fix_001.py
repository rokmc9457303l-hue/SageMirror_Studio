...
def render_right_gemma_panel():
    ...
    /* 둥근 통합 박스 스타일 - border wrapper 기준 */
    ...
    /* 요소 간격 없애기 */
    div[data-testid="stVerticalBlockBorderWrapper"]:has(#chatbar-marker) div[data-testid="stVerticalBlock"] {
        gap: 0 !important;
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    # 2. 하단 통합 박스 (border=True로 감싸서 CSS 타겟팅)
    chatbar_container = st.container(border=True)
    with chatbar_container:
        ...
        # 둘째 줄: ＋, 빈공간, 모델, ↑
        c_plus, c_space, c_model, c_send = st.columns([1, 4.5, 3.5, 1])
