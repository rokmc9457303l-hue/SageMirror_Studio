...
🪞 현자의 거울 스튜디오 — Master App v15.9.26

[v15.9.26 업데이트 사항: 2026-05-23]

- 좌측 Sidebar 최상단에 🎬 Episode Control Center 안전 추가
- 회차(Episode)별 독립 아웃풋 및 세션 관리 구조 구현
- 💾 현재 Episode 전체 저장 및 🆕 새 Episode 시작 기능 추가
- 에피소드 아웃풋을 01_Librarian~08_Dashboard 폴더로 세분화하여 Outputs와 Obsidian에 이중 저장 기능 구현
- 각 파트별 렌더링 시 최상단 타이틀에 현재 Episode 번호가 동적으로 추가되도록 전역 CSS 설계 주입
...
def save_episode_obsidian_session(episode):
    try:
        ...
        filepath = os.path.join(session_dir, f"session_{ts}_v15.9.26.md")
...
"""
        ...
        hist_filepath = os.path.join(hist_dir, f"session_{ts}_{episode}_v15.9.26.md")
...
with st.sidebar:
    ...
    with st.container():
        ...
        selected_ep = st.selectbox(
            ...
            ep_list,
            index=ep_idx,
            key="ecc_episode_select"
        )
        
        if selected_ep != current_ep:
    ...
    st.markdown(f"### {APP_TITLE} **v15.9.26**")
