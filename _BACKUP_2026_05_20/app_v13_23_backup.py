# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 — Master App v13.16.1
[v13.16.1 업데이트 사항: 2026-05-19]
- UI 이모지 [LIST] -> 📋 치환 작업 완료 (전체 11개 라인 정밀 교체)
"""
...
def generate_research_draft(channel_url, topic, gemma_protocol, master_prompt):
    obsidian_context = ""
    try:
        search_result = search_obsidian(topic)
...
col_r_view, col_r_copy, col_r_save, col_r_obsidian = st.columns([1.2, 1.2, 1.2, 1.6])
with col_r_view:
    if st.button("👁 보기", use_container_width=True, key="research_view_btn"):
        popup_edit_research_p2()

with col_r_copy:
    if st.button("📋 복사", use_container_width=True, key="research_copy_btn"):
...
with col_r_save:
    if st.button("💾 저장", use_container_width=True, key="research_save_btn"):

        ts = datetime.now().strftime("%Y%m%d_%H%M%S")

        save_dir = os.path.join("Backups", "Part1_Research")

        os.makedirs(save_dir, exist_ok=True)

        save_path = os.path.join(save_dir, f"research_{ts}.md")

        with open(save_path, "w", encoding="utf-8") as f:
            f.write(st.session_state.p2_research_result)
