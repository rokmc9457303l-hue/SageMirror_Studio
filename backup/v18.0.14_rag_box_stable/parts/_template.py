...
def render_packet_tab(part_num: int):
    ...
    if not existing_packet:
        if st.button("📦 전달 패킷 생성", key=f"p{part_num}_packet_create", type="primary"):
            ...
            # 백그라운드 큐로 저장 위임 (UI 즉시 반응)
            from core.auto_save import schedule_part_save
            schedule_part_save(
                part_num=part_num,
                content=str(result),
            ...
            st.success("✅ 패킷 생성 완료 — 옵시디언 저장은 백그라운드 진행 중")
            save_workspace()
            st.rerun()
    else:
