...
def run_research(topic: str, use_tavily: bool = True) -> str:
    rag = search_rag(topic, max_files=8, max_chars=500)
    
    tavily_data = ""
    if use_tavily:
        tavily_key = get_state("api_tavily", "")
        if tavily_key:
            try:
                resp = requests.post(
...
[옵시디언 RAG 자료]
{rag[:1500] if rag else '(관련 자료 없음 — 신규 주제)'}

[웹 검색 보조 자료]
{tavily_data[:1000] if tavily_data else '(웹 검색 사용 안 함)'}

위 자료를 바탕으로 다음 항목을 정리하라:
1. 핵심 자료 5~10개 [SOURCE: ...] 표기 필수
2. 감정 키워드 추출
3. 철학·심리 연결점
4. 성경 연결점 (가능 시)

옵시디언 RAG 자료가 부족한 부분은 [NEED_RESEARCH: 키워드]로 표기하라.
"""
    return call_model(prompt=user_query, system=prompt, model=get_state("current_model"))
