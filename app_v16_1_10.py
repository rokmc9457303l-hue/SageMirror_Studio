...
🪞 현자의 거울 스튜디오 — Master App v16.1.10

[v16.1.10 업데이트 사항: 2026-05-26]
- Part 2 Step 1 UI State Management Patch (상태 관리 버그 패치)
- 프롬프트 초기화 덮어쓰기 조건문 수정, 젬마 프로토콜 및 썸네일 기획 직접 편집 양방향 동기화 복구
...
def render_part2():
    ...
    for _k, _v in _p2_prompts.items():
        if not st.session_state.get(_k):  # 값이 비어있거나 없을 때만 기본값 적용
    ...
    with c_left:
        ...
        p2_proto_val = st.text_area("젬마 프로토콜", value=st.session_state.get("p2_gemma_protocol", ""), height=270, label_visibility="collapsed", key="p2_gemma_protocol_widget", disabled=is_locked)
        if p2_proto_val != st.session_state.get("p2_gemma_protocol", ""):
            st.session_state.p2_gemma_protocol = p2_proto_val
            save_workspace_state()
...
"""
        ...
        thumb_val = st.text_area("썸네일 기획 결과 (수정 가능)", value=st.session_state.get("p2_thumbnail_plan", ""), height=195, label_visibility="collapsed", key="p2_thumb_area_widget", disabled=is_locked)
        if thumb_val != st.session_state.get("p2_thumbnail_plan", ""):
            st.session_state.p2_thumbnail_plan = thumb_val
            save_workspace_state()
