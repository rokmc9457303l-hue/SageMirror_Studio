# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's 거울 스튜디오) — Master App v7.9
================================================================
[v7.9 업데이트 사항: 2026-05-11 19:00]
- UI 구조 개선: 하단 3칸의 가로 구분선을 없애고 세로 블록형 카드(Box)로 명확히 분리
- 팝업 지원 확대: 자료조사 결과와 총괄기획안에 대형 팝업(스크롤, 복사/붙여넣기) 버튼 추가
- 텍스트 영역의 가독성 향상 및 조작 편의성 극대화
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v7.9",
    page_icon="🪞",
    layout="wide",
...
# 2. 세션 상태 초기화 (v7.9)
# =====================================================================
DEFAULT_OBSIDIAN_RULES_V79 = """[옵시디언 지식 구조화 규칙서]
...
MASTER_RESEARCH_PROMPT_V79 = """[자료 조사 파트 전용 마스터 규칙서: 절대 가이드]
...
GEMMA_PROTOCOL_V79 = """[Gemma4 행동 및 작업 지침서 (Gemma Protocol)]
...
def init_session_state():
    defaults = {
        ...
        "tavily_api_key": "",
        "obsidian_rules": DEFAULT_OBSIDIAN_RULES_V79,
        "base_prompt_rules": MASTER_RESEARCH_PROMPT_V79,
        "obsidian_history": [],
        "prompt_history": [],
        "popup_history": [],
        ...
        "p1_gemma_protocol": GEMMA_PROTOCOL_V79,
    ...
    if st.session_state.base_prompt_rules != MASTER_RESEARCH_PROMPT_V79 and not st.session_state.get("p1_saved"):
        st.session_state.base_prompt_rules = MASTER_RESEARCH_PROMPT_V79
    if st.session_state.obsidian_rules != DEFAULT_OBSIDIAN_RULES_V79 and not st.session_state.get("p1_saved"):
        st.session_state.obsidian_rules = DEFAULT_OBSIDIAN_RULES_V79
    if st.session_state.p1_gemma_protocol != GEMMA_PROTOCOL_V79 and not st.session_state.get("p1_saved"):
        st.session_state.p1_gemma_protocol = GEMMA_PROTOCOL_V79
...
# =====================================================================
def render_login():
    st.markdown(f"<h1 style='text-align:center'>{APP_TITLE} <span style='color:#10B981;font-size:0.5em;'>v7.9 View Edition</span></h1>", unsafe_allow_html=True)
...
# =====================================================================
# V7.9 신규 팝업 로직 (Gemma Protocol 및 결과창들)
# =====================================================================
            ...
            if st.session_state.p1_planning_result:
                ...
                if st.button("🔒 최종안 옵시디언 자동 백업", type="primary", use_container_width=True):
                    ...
                    if st.session_state.path_obsidian:
                        ...
                        md = f"""# [[{topic_title}]]
## 📌 Brief Summary
Sage Mirror Studio v7.9 총괄 시나리오 기획안
...
- **Projects/Contexts:** [[SageMirror_Production_v7.9]]
