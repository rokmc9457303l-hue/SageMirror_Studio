# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's 거울 스튜디오) — Master App v7.8
================================================================
[v7.8 업데이트 사항: 2026-05-11 18:50]
- 채널 탐색 고도화: 복수(5개)의 최적 채널을 검색하여 리스트로 제공 후 선택 매칭 (정확도 상승)
- 버튼 UX 개선: 하단 3칸의 버튼 잠금을 해제하고, 순서가 맞지 않을 경우 친절한 에러 메시지(경고창) 출력
- 텍스트 영역: disabled 제거 및 UX 안정성 강화
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v7.8",
    page_icon="🪞",
    layout="wide",
...
# 2. 세션 상태 초기화 (v7.8)
# =====================================================================
DEFAULT_OBSIDIAN_RULES_V78 = """[옵시디언 지식 구조화 규칙서]
...
MASTER_RESEARCH_PROMPT_V78 = """[자료 조사 파트 전용 마스터 규칙서: 절대 가이드]
...
GEMMA_PROTOCOL_V78 = """[Gemma4 행동 및 작업 지침서 (Gemma Protocol)]
...
def init_session_state():
    defaults = {
        ...
        "tavily_api_key": "",
        "obsidian_rules": DEFAULT_OBSIDIAN_RULES_V78,
        "base_prompt_rules": MASTER_RESEARCH_PROMPT_V78,
        "obsidian_history": [],
        "prompt_history": [],
        "popup_history": [],
        ...
        "p1_gemma_protocol": GEMMA_PROTOCOL_V78,
    ...
    if st.session_state.base_prompt_rules != MASTER_RESEARCH_PROMPT_V78 and not st.session_state.get("p1_saved"):
        st.session_state.base_prompt_rules = MASTER_RESEARCH_PROMPT_V78
    if st.session_state.obsidian_rules != DEFAULT_OBSIDIAN_RULES_V78 and not st.session_state.get("p1_saved"):
        st.session_state.obsidian_rules = DEFAULT_OBSIDIAN_RULES_V78
    if st.session_state.p1_gemma_protocol != GEMMA_PROTOCOL_V78 and not st.session_state.get("p1_saved"):
        st.session_state.p1_gemma_protocol = GEMMA_PROTOCOL_V78
...
# =====================================================================
def render_login():
    st.markdown(f"<h1 style='text-align:center'>{APP_TITLE} <span style='color:#10B981;font-size:0.5em;'>v7.8 Accuracy Edition</span></h1>", unsafe_allow_html=True)
...
def render_part1():
    ...
    with c_right:
        ...
        if st.session_state.p1_channel_search_results:
            with st.container(border=True):
                ...
                if selected_channel:
                    # ' - ' 기준으로 URL 부분만 파싱하여 p1_channel_url에 주입
    ...
    with c_bench:
        ...
        # 버튼을 활성화(disabled=is_locked)하되, URL이 없을 경우 에러 메시지를 띄우는 UX로 변경
    ...
    with c_research:
        ...
        # v7.8 UX: 버튼 강제 비활성화 해제 -> 클릭 시 친절한 에러 발생
        ...
        if st.session_state.p1_planning_result:
            ...
            if st.button("🔒 최종안 옵시디언 자동 백업", type="primary", use_container_width=True):
                ...
                if st.session_state.path_obsidian:
                    ...
                    md = f"""# [[{topic_title}]]
## 📌 Brief Summary
Sage Mirror Studio v7.8 총괄 시나리오 기획안
...
- **Projects/Contexts:** [[SageMirror_Production_v7.8]]
