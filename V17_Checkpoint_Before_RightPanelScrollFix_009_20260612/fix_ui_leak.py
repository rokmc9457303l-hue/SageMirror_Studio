...
# 2. Remove "RUNNING VERSION"
content = re.sub(r'st\.caption\("RUNNING VERSION:.*?"\)', '', content)

# 3. Check for "하단 입력 바" labels
# The label "하단 입력 바 및 기능" was in the previous render_right_gemma_panel()
# Since we replaced the whole function, it's gone!
# Just in case, let's make sure it's not anywhere else.
content = content.replace('st.markdown("<br>", unsafe_allow_html=True)\n    st.markdown("하단 입력 바", unsafe_allow_html=True)', '')

with open('app_v17_2_3.py', 'w', encoding='utf-8-sig') as f:
    f.write(content)
print("UI updated successfully without CSS leak and RUNNING VERSION removed.")
