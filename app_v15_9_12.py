...
🪞 현자의 거울 스튜디오 — Master App v15.9.12

[v15.9.12 업데이트 사항: 2026-05-23]

- 리서치 자동저장 상태 실시간 확인 LED 인디케이터 UI 추가 (공통 옵시디언 검색란 우측)

- 구글/Tavily 자동 및 수동 리서치 저장 성공 여부 시각적 피드백 제공 (대기 시 White, 성공 시 Blue)
...
"""
    ...
    with st.expander("🔍 옵시디언 감정 기반 검색 — Part 2 참조용 (클릭하여 열기)", expanded=False):

        st.caption("철학·성경·에세이 자료를 옵시디언에서 먼저 검색하여 분석 품질을 높이세요.")

        DEFAULT_P2_TAGS = (
            "고독, 후회, 상실, 관계, 용서, 공허, 의미상실, "
            "쇼펜하우어, 칼 융, 빅터 프랭클, 스토아, 몽테뉴, "
            "에세이, 다크심리학, 시편, 잠언, 전도서, 욥기, 성경"
        )

        p2_obs_query = st.text_input(
            "감정 / 철학 / 성경 / 에세이 키워드 검색",
            value=st.session_state.get("p2_obsidian_query_val", DEFAULT_P2_TAGS),
            placeholder="예: 고독, 시편, 쇼펜하우어, 용서",
            key="p2_obsidian_search_query"
        )
        st.caption("💡 기본 태그 자동 입력 — 직접 수정 가능")

        if st.button("🔎 옵시디언 검색 (Part 2 참조)", use_container_width=True, key="p2_obsidian_search_btn"):
            try:
                p2_results = simple_keyword_search(
                    st.session_state.get("path_obsidian", ""),
                    p2_obs_query,
                    top_k=10
                )
                if not p2_results:
                    ...
                    st.session_state.pipeline_state["p2_obsidian_search_results"] = p2_results
                    st.session_state.p2_obsidian_search_results = p2_results
                    st.success(f"✅ {len(p2_results)}개 자료 발견 — 젬마가 분석 시 참조합니다.")
                    for r in p2_results:
                        with st.container(border=True):
                            st.markdown(f"### 📘 {r['title']}")
                            st.caption(f"점수: {r['score']}")
...
def render_part34():
    ...
    with st.expander("🔍 옵시디언 감정 기반 검색 — 대본 참조용 (클릭하여 열기)", expanded=False):

        st.caption("대본 작성 전 성경·철학·에세이 자료를 옵시디언에서 먼저 검색하세요.")
        ...
        p3_obs_query = st.text_input(
            "감정 / 철학 / 성경 / 에세이 키워드 검색",
            value=st.session_state.get("p3_obsidian_query_val", DEFAULT_P3_TAGS),
            placeholder="예: 고독, 시편, 쇼펜하우어, 용서",
            key="p3_obsidian_search_query"
        )
        st.caption("💡 기본 태그 자동 입력 — 직접 수정 가능")

        if st.button("🔎 옵시디언 검색 (대본 참조)", use_container_width=True, key="p3_obsidian_search_btn"):
            try:
                p3_results = simple_keyword_search(
                    st.session_state.get("path_obsidian", ""),
                    p3_obs_query,
                    top_k=10
                )
                if not p3_results:
                    ...
                    st.session_state.pipeline_state["p3_obsidian_search_results"] = p3_results
                    st.success(f"✅ {len(p3_results)}개 자료 발견 — 젬마가 대본 작성 시 참조합니다.")
                    for r in p3_results:
                        with st.container(border=True):
                            st.markdown(f"### 📘 {r['title']}")
                            st.caption(f"점수: {r['score']}")
