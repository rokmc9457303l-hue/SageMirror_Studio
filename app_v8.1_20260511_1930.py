# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's 거울 스튜디오) — Master App v8.1
================================================================
[v8.1 업데이트 사항: 2026-05-11 19:30]
- 공간 효율화: 상단의 빈 공간을 활용하여 Part 1 타이틀과 PIN 로그인 창을 세련되게 배치
- 거추장스러웠던 Expander(접이식 메뉴) 형태의 비밀번호 창을 상단 가로바 형태의 로그인 폼으로 디자인 변경
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v8.1",
    page_icon="🪞",
    layout="wide",
...
# 2. 세션 상태 초기화
# =====================================================================
DEFAULT_OBSIDIAN_RULES_V81 = """[옵시디언 지식 구조화 규칙서]
...
# =====================================================================
def render_login():
    st.markdown(f"<h1 style='text-align:center'>{APP_TITLE} <span style='color:#10B981;font-size:0.5em;'>v8.1 Design Edition</span></h1>", unsafe_allow_html=True)
...
def render_part2():
    ...
    with c_right:
        st.markdown("##### 🔍 떡상 채널 발굴용 탐색기")
        st.caption("AI 카피를 배제한 순수 인간 운영의 최고 조회수 채널 여러 개를 검색하여 검토합니다.")
        search_kw = st.text_input("검색 키워드 (가이드라인 기반 자동 세팅)", value="50대 심리 철학 위로 채널", disabled=is_locked, key="p2_search_kw")
        
        if st.button("🌐 전 세계 채널 5개 탐색 및 리스트업 (Tavily)", disabled=is_locked, use_container_width=True, key="p2_tav_btn"):
            if not st.session_state.tavily_api_key:
                st.error("좌측 사이드바 '⚙️ 설정 변경'에서 Tavily API Key를 먼저 입력하세요.")
            else:
                with st.spinner("최고 떡상 원본 채널 5개를 심층 탐색 중..."):
                    ...
                    if "error" in res: st.error(res["error"])
                        ...
                        st.session_state.p2_channel_search_results = res.get("results", [])
                        st.success("🎯 채널 검색 완료! 아래 목록에서 가장 적합한 채널을 선택하세요.")
        
        if "p2_channel_search_results" in st.session_state and st.session_state.p2_channel_search_results:
            with st.container(border=True):
                st.markdown("**🎯 분석할 채널을 선택하세요 (선택 시 아래 URL에 자동 입력됨):**")
                options = []
                for i, r in enumerate(st.session_state.p2_channel_search_results):
                    options.append(f"[{i+1}] {r.get('title', '제목없음')} - {r.get('url', '#')}")
                
                selected_channel = st.radio("검색된 채널 리스트", options, label_visibility="collapsed", disabled=is_locked, key="p2_radio_chan")
                
                if selected_channel:
                    selected_url = selected_channel.split(" - ")[-1]
                    st.session_state.p2_channel_url = selected_url
        
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("##### 🎯 분석 대상 확정")
        if "p2_channel_url" not in st.session_state: st.session_state.p2_channel_url = ""
        st.session_state.p2_channel_url = st.text_input("타겟 유튜브 URL (위에서 선택 시 자동 입력됨)", value=st.session_state.p2_channel_url, disabled=is_locked, key="p2_input_url")
        if "p2_region" not in st.session_state: st.session_state.p2_region = "국내+국외 모두"
        st.session_state.p2_region = st.selectbox("타겟 지역", ["국내+국외 모두", "국내 우선", "국외 우선"], disabled=is_locked, key="p2_region_sel")
