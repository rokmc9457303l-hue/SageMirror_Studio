...
- 썸네일 3세트 다이얼로그(popup_thumbnail_selector) 호출 방식 플래그 기반(st.session_state.show_p2_thumb_popup)으로 갱신하여 렌더링 꼬임 및 사라지지 않는 현상 해결
...
def render_part2():
    if st.session_state.get("show_p2_thumb_popup", False):
        st.session_state.show_p2_thumb_popup = False
    ...
    with c_left:
        ...
        gemma_proto_val = st.text_area("젬마 프로토콜 (수정은 편집 버튼 클릭)", value=st.session_state.get("p2_gemma_protocol", ""), height=270, label_visibility="collapsed", key="p2_gemma_protocol_widget")
        if gemma_proto_val != st.session_state.get("p2_gemma_protocol", ""):
            st.session_state.p2_gemma_protocol = gemma_proto_val
            save_workspace_state()
        ...
        if st.button("🚀 썸네일/주제/제목 3세트 생성 (AI)", use_container_width=True, disabled=is_locked, key="p2_thumb_btn"):
            ...
            if "p2_research_result" not in st.session_state or not st.session_state.p2_research_result:
                ...
                with st.spinner("썸네일 3세트 생성 중..."):
                    ...
                    st.session_state.show_p2_thumb_popup = True
                    save_workspace_state()
