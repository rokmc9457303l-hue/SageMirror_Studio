...
def _call_gemma_fast_a_mode(prompt: str, system: str = "", model: str = "gemma4:e2b") -> str:
    """
    [A모드 전용] 빠른 대화를 위해 RAG, Tavily, sidebar, st.fragment 내부 렉 유발 요소를
    모두 완전 차단하고, Ollama API에 직접 num_predict 제한 옵션을 전달하여 1~2초 이내 초고속 반환.
    """
    try:
        ...
        resp = _req.post(ollama_url, json=payload, timeout=60)
        ...
        if not content or not content.strip():
            ...
            if thinking and thinking.strip():
                clean_think = thinking.replace("Thinking Process:\n\n", "").strip()
                if len(clean_think) > 150:
                    clean_think = clean_think[:147] + "..."
                return clean_think
            ...
            if st.session_state.get("pending_stream"):
                ...
                if current_mode == "A":
                    fast_chat_instruction = (
                        ...
                        "4. [중요] 생각 과정(Thinking Process / <think> 등)을 완전히 생략하고 즉시 최종 답변만 출력하라.\n"
                        "You must NOT output any Thinking Process or reasoning. Output ONLY the final short conversational response immediately.\n"
                        "예: '안녕하세요. 어떤 작업을 이어가면 될까요?'"
                    )
                    sys_ctx += fast_chat_instruction
