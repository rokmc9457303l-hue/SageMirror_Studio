# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 — Master App v15.8
[v15.8 업데이트 사항: 2026-05-22]
- 파트 5 (영상 생성) 탭 구조 전면 개점 및 3단 스텝 가로 배치 레이아웃 구현 (수신, 4단 매핑 조립, Opal 12씬 이하 계정 분배)
- 외부 격리 회차별 자동 저장 파이프라인 (C:\SageMirror_Outputs) 구축 및 파트 3~6 저장 연동
"""
...
def render_part6_video():
    ...
    with col3:
        ...
        if not valid_scenes:
            ...
            if st.button("💾 [Part 5] 오팔 배분 외부 저장 및 백업", type="primary", use_container_width=True, key="p6_step3_outputs_save_btn"):
                csv_filename = f"video_dispatch.csv"
                ...
                if saved_path:
                    ...
                    save_workspace_state()
                    st.toast("✅ [영상 파트] 오팔 배분 저장 완료!", icon="💾")
                    
                    success, msg = auto_git_push(f"Part5 Video dispatch auto backup: {st.session_state.episode_name}")
                    if success:
                        st.toast("🚀 GitHub 백업 완료!", icon="🚀")
                    else:
    ...
    if st.button("💾 [파트 5: 영상 생성] 옵시디언 자동 백업", type="primary", use_container_width=True, key="p6_video_obsidian_backup_btn"):
        try:
            ...
            title = f"{ep} 영상 생성 오팔 배분 결과"
            
            # 마크다운 내용 조립
            obsidian_content = f"# {title}\n\n"
            obsidian_content += f"- 회차: {ep}\n"
            ...
            if dist_data:
                ...
                for row in dist_data:
                    obsidian_content += f"| {row['계정']} | {row['씬번호']} | {row['이미지파일']} | {row['동영상파일']} |\n"
            
            save_path = save_obsidian_memory("05_Video_Opal", title, obsidian_content)
            if save_path:
                st.session_state.p6_video_outputs_saved = True
                save_workspace_state()
                st.toast("✅ [파트 5: 영상 생성] 저장 완료!", icon="💾")
                
                success, msg = auto_git_push(f"Part5 Video Obsidian Backup: {ep}")
                if success:
                    st.toast("🚀 GitHub 백업 완료!", icon="🚀")
                else:
                    st.warning(f"Git Push 실패: {msg}")
        except Exception as e:
            st.error(f"[옵시디언 백업] 에러 발생: {e}")
