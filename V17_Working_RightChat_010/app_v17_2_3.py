...
with st.sidebar:
    ...
    idx = 0
    for i, p in enumerate(part_options):
        if cur_p == p: idx = i
    part = st.radio("이동할 파트", part_options, index=idx)
...
def render_right_gemma_panel():
    ...
    div[data-testid="column"]:has(#right-gemma-fixed-marker) {
        ...
        with st.popover("＋ 기능", use_container_width=True):
            if st.button("💾 마지막 정상 결과 저장", use_container_width=True):
                ...
                if last_success_msg:
                    ...
                    cat = "RightResearch"
                    _save_raw_wiki(last_success_msg["content"], title=f"Research_{ts}", source_type="RightResearch", part_key="part0", model_name=last_success_msg["engine"])
    ...
    if send_clicked and user_input.strip():
        ...
        with chat_container.chat_message("assistant"):
            spinner_msg = "로컬 모델을 준비하고 있습니다. 최초 실행은 최대 2분 정도 걸릴 수 있습니다." if selected_mode in ["gemma4:e2b", "gemma4:e4b"] else "응답 대기 중..."
            with st.spinner(spinner_msg):
                ...
                # 답변 성공, 실패 여부와 상관없이 무조건 입력창 초기화 플래그 설정
...
with main_col:
    packet = st.session_state.get("right_gemma_push_packet")
    if packet and part.startswith(packet.get("target_part", "")):
        st.info(f"📬 우측 젬마에서 보강 자료가 도착했습니다! (엔진: {packet.get('engine')})")
        with st.expander("도착한 보강 자료 보기", expanded=True):
            st.text_area("보강 자료 내용", value=packet.get("result", ""), height=150, disabled=True)
            if st.button("검토 완료 (닫기)", key="rg_packet_close_btn"):
                st.session_state["right_gemma_push_packet"] = None
                st.rerun()
