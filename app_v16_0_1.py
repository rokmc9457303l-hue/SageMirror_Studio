...
🪞 현자의 거울 스튜디오 — Master App v16.0.1

[v16.0.1 업데이트 사항: 2026-05-25]
- [A] References Memory 주입 품질 최적화 함수(build_condensed_reference_context) 신규 구현
- [B] RAG 파일 필터링 추가: 본문 300자 미만 제외, 오염 문자열(span, display, background-color, 자동저장:, [EP001]) 제외, 메타데이터 부재 제외
- [C] 중복 파일명 필터링(mtime 기준 최신만 사용) 및 전역 위키링크 중복 제거 후 병합 제공
- [D] 최근 8개 파일은 본문 일부(최대 800자) 포함, 9번째 이상 오래된 파일은 본문 생략 자동 축약 적용
- [E] RAG 예외 처리 강화 및 주입 상태 표시 개선 ("🧠 References Memory Loaded: X files" st.caption)
- [F] RUN_APP.bat, RUN_DEBUG.bat, RUN_APP.vbs의 실행 타깃을 app_v16_0_1.py로 최신화
...
def clean_prompt_contamination(text: str) -> str:
    ...
    for target in targets:
        ...
        # 3차 방어: span style 블록이 줄 단위로 남은 경우 전체 CSS 잔재 제거
        text = _re.sub(
        ...
        # 4차 방어: style 속성 내부 CSS 잔재가 텍스트로 풀려 남은 경우 제거
...
def sanitize_workspace_prompt_values_once():
    try:
        workspace_path = Path(WORKSPACE_STATE_FILE)
        ...
        if changed:
            ...
            print("[SANITIZE] workspace_state.json 정화 완료")
...
def build_condensed_reference_context(memory_items: list, max_chars: int = 30000) -> str:
    """
    RAG Memory Optimization Layer (v16.0.1)
    ...
    - 추가 안전 조건:
      ...
      4. 동일 파일명 중복 시 최신 파일만 사용 (mtime 기준 최신만 사용)
      5. 제외 시 st.caption 출력
    """
    import re
    import streamlit as st
    ...
    try:
        ...
        for idx, item in enumerate(unique_memory_items):
            ...
            if any(pw in content for pw in polluted_keywords):
                st.caption(f"⚠️ 오염 가능 Reference 제외: {filename}")
                continue
                
            parsed = parse_markdown_reference_v2(content)
            
            if len(parsed["body"]) < 300:
                st.caption(f"⚠️ 오염 가능 Reference 제외: {filename} (본문 300자 미만)")
                continue
            ...
            if src_empty and kw_empty and wiki_empty:
                st.caption(f"⚠️ 오염 가능 Reference 제외: {filename} (메타데이터 없음)")
                continue
        ...
        return f"[References Memory 최적화 중 오류 발생: {e}]"
...
def load_workspace_state():
    ...
    # 정화된 값이 있으면 workspace_state.json에 즉시 반영
    try:
        with open(WORKSPACE_STATE_FILE, "w", encoding="utf-8") as f:
