...
🪞 현자의 거울 스튜디오 — Master App v17.1.7

[v17.1.7 업데이트 사항: 2026-05-29]
- 파트 0: 🤖 젬마 스튜디오 풀스크린 2단 레이아웃 개편
- 구글 에코시스템(Docs/Drive/Web) 텍스트 파서 탑재 및 연동
- 가로 칩 모델 선택기(E2B, E4B, Gemini, Tavily) 및 네이티브 채팅 이식
- 기존 팝업 어시스턴트(popup_assistant)를 파트 0 이동 안내로 몽키 패치 리다이렉트
...
from sage_popups_v17_1_7 import (

    popup_edit_obsidian, popup_edit_prompt, popup_assistant as _orig_popup_assistant,

)

# ── [몽키 패치] 팝업 어시스턴트 리다이렉션 ──
@st.dialog("🤖 젬마 어시스턴트 안내", width="medium")
def popup_assistant_redirected():
    st.warning("⚠️ 이제 젬마 어시스턴트는 [파트 0: 🤖 젬마 스튜디오]에서 풀스크린으로 실행됩니다.")
    st.info("이동 버튼을 누르시면 젬마 스튜디오 화면으로 즉시 전환됩니다.")
    if st.button("🤖 젬마 스튜디오로 이동", use_container_width=True, key="p0_redirect_btn"):
        st.session_state.sidebar_part = "파트 0: 🤖 젬마 스튜디오 (대화 & 자료수집)"
        st.rerun()

popup_assistant = popup_assistant_redirected
