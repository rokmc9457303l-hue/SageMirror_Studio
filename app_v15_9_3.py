...
🪞 현자의 거울 스튜디오 — Master App v15.9.3

[v15.9.3 업데이트 사항: 2026-05-22]

- 파트 1 채널 탐색기 AI 분석 및 자동 1순위 확정 시스템 고도화

- 인간운영 채널 5개 / AI제작 채널 5개 균형 수집 및 트렌드/떡상 지표 기준 1순위 자동 선정

- 파트 2 Alchemist로의 자동 데이터 상속 및 화면 세션 Rerun 이동 연동
...
def init_session_state():
    ...
    defaults = {
        ...
        "p1_gemini_model": "gemini-2.5-pro",
        "p1_channel_top10": [],
    ...
    with c_obs:
        ...
        if obs_ok:
            ...
            st.markdown(
                ...
                '<span class="stat-label"><span style="color:#f87171;">⬤</span>  옵시디언 RAG</span>'
...
def analyze_channel_to_topics(channel, region, obsidian_rules):

    base = f"""당신은 유튜브 채널 분석 전문가입니다.
채널: {channel}을 분석하여 핵심 주제 20개를 추출하십시오.
                    ...
                    def _gemini_call(model, prompt, timeout=45):
                        with _cf.ThreadPoolExecutor(max_workers=1) as ex:
                            ...
                            try:
                                ...
                                raise Exception(f"⏱ 제미나이 응답 시간 초과 ({timeout}초) — 다른 모델로 전환하거나 다시 시도하세요.")
