...
🪞 현자의 거울 스튜디오 — Master App v15.9.19
...
def save_workspace_state():
    ...
    for k in secrets_keys:
        ...
        if k in st.session_state:
            ...
            secrets_data[k] = st.session_state[k]
    ...
    try:
        ...
        if secrets_data:
            ...
            with open(LOCAL_SECRETS_FILE, "w", encoding="utf-8") as f:
...
with st.sidebar:
    ...
    st.markdown(f"### {APP_TITLE} **v15.9.19**")
...
def lock_and_push_final_version(part_num, display_name, keys_to_backup):
    """현재 파트 결과물을 Final_LOCK으로 저장 후 GitHub Push"""
    try:
        obs_path = st.session_state.get("path_obsidian", "")
        if not obs_path:
...
"""
    ...
    # ── 🔒 최종본 Lock & 수정본 생성 버튼 ──────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    _cl1, _cr1 = st.columns(2)
    with _cl1:
        if st.button("🔒 Part 1 최종본 Lock & GitHub Push",
                     key="p1_lock_btn", use_container_width=True):
            lock_and_push_final_version(1, "벤치마킹 & 자료조사", ["p1_research_result","p1_planning_result"])
    with _cr1:
        if st.button("🔓 Part 1 수정본 생성",
                     key="p1_rev_btn", use_container_width=True):
...
# Part 6 — render_part6_opal() 메인 함수
    ...
    # ── 🔒 최종본 Lock & 수정본 생성 버튼 ──────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    _cl34, _cr34 = st.columns(2)
    with _cl34:
        if st.button("🔒 Part 34 최종본 Lock & GitHub Push",
                     key="p34_lock_btn", use_container_width=True):
            lock_and_push_final_version(34, "대본 & 이미지", ["p34_script_result","p4_image_result"])
    with _cr34:
        if st.button("🔓 Part 34 수정본 생성",
                     key="p34_rev_btn", use_container_width=True):
            create_revision_version(34, "대본 & 이미지", ["p34_script_result","p4_image_result"])
