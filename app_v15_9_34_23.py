...
🪞 현자의 거울 스튜디오 — Master App v15.9.34.23

[v15.9.34.23 업데이트 사항: 2026-05-25]
- [A] 10차 결합 결과 및 call_gemma()를 안전하게 감싸서 호출하는 call_gemma_safe_memory() 래퍼 함수 추가 (예외 처리 기본 탑재)
- [B] 메인 페이지 하단 개발자 테스트 UI에 "✏️ Safe Memory Wrapper 테스트 질문" 입력창 및 "🛡️ Safe Memory Wrapper 테스트" 수동 버튼 컴포넌트 추가
- [C] 버튼 클릭 시에만 1회성 call_gemma_safe_memory() 호출 및 debug_safe_memory_response에 결과 반환
- [D] sage_popups.py의 save_workspace_state 임포트 경로를 최신 버전(app_v15_9_34_23)으로 업데이트
- [E] RUN_APP.bat 및 RUN_DEBUG.bat 실행 대상을 app_v15_9_34_23.py로 변경
...
# ══════════════════════════════════════════════════════════════
# 🧠 RAG 자동 카테고리 감지 + 파트별 태그 저장 시스템 v1.0
# ══════════════════════════════════════════════════════════════
...
[요약 및 검수]:"""
                ...
                md_content += f"## 🔎 검색 키워드\n{query_val}\n\n"
                md_content += f"## 🤖 사용 모델\n{selected_model}\n\n"
                md_content += build_rag_classification_markdown(detected_rag) + "\n"
                ...
                md_content += "## 🔗 연결 개념\n"
                for t in detected_rag.get("keywords", [])[:20]:
                    md_content += f"- [[{t}]]\n"
                md_content += "\n"
...
def clean_prompt_contamination(text: str) -> str:
    ...
    # span tag block 전체를 제거하는 정규식 (큰따옴표/작은따옴표 처리)
    ...
    try:
        ...
        if suffix in [".txt", ".md"]:
            try:
                ...
                # UTF-8, CP949, EUC-KR 순차적 폴백 디코딩 시도
        ...
        # 2. HTML 파일 처리
            ...
            try:
                ...
                try:
                    ...
                    # BeautifulSoup 로드 또는 파싱 실패 시 정규식 등으로 대체 추출 시도
                    import re as _re
...
def convert_text_to_markdown_structure(raw_text: str, source_name: str = "unknown") -> str:
    ...
    # - 연속 공백 제거 (2개 이상의 공백을 1개로)
    text = _re.sub(r'[ \t]+', ' ', text)
    
    # - 특수문자 반복 제거 (예: ===, ---, ***, ___ 등이 3개 이상 반복되는 것을 3개로 통일)
    text = _re.sub(r'([=\-*_#~])\1{2,}', r'\1\1\1', text)
    
    # - 너무 긴 공백 줄 정리 및 3줄 이상 빈줄 제거 (빈 줄 2개 이하로 제한)
    text = _re.sub(r'[ \t]+\n', '\n', text)
    text = _re.sub(r'\n{3,}', '\n\n', text)
...
def build_all_parts_common_tags_preview(detection: dict) -> dict:
    """PART_RAG_TAG_MAP의 part1~part8 태그를 모두 모으고 detection 결과와 합쳐 공용 태그 미리보기를 빌드합니다."""
    import re as _re
    
    # 1. part1~part8 태그 모두 수집 (순서 유지)
    all_tags = []
    for i in range(1, 9):
        part_key = f"part{i}"
        all_tags.extend(PART_RAG_TAG_MAP.get(part_key, []))
    all_part_tags = _unique_keep_order(all_tags, limit=80)
    
    # 2. detected_keywords
    detected_keywords = detection.get("keywords", [])
    
    # 3. combined_keywords = all_part_tags + detected_keywords (중복 제거, 120개 제한)
    combined_keywords = _unique_keep_order(all_part_tags + detected_keywords, limit=120)
    
    # 4. wiki_links = all_part_tags 위키링크화 + detection 위키링크 (중복 제거, 120개 제한)
    all_part_wiki = [f"[[{tag}]]" for tag in all_part_tags]
    detected_wiki = detection.get("wiki_links", [])
    wiki_links = _unique_keep_order(all_part_wiki + detected_wiki, limit=120)
    
    # 5. hash_tags = all_part_tags 해시태그화 + detection 해시태그 (중복 제거, 120개 제한)
    def clean_hashtag(tag_str):
        tag_str = str(tag_str).strip()
        if not tag_str:
            return ""
        # 공백을 _ 로 바꾼다
        cleaned = _re.sub(r"\s+", "_", tag_str)
        # 만약 맨 앞이 #이 아니면 붙인다
    ...
    hash_tags = _unique_keep_order(all_part_hash + detected_hash, limit=120)
...
def save_reference_markdown_file(
    ...
    """RAG 공용 태그 및 메타데이터를 포함한 마크다운 문서를 References 폴더에 저장합니다."""
    import os
    import re
    from datetime import datetime
    from pathlib import Path
...
def load_recent_reference_files(max_files: int = 20, max_chars: int = 120000) -> list:
    ...
    try:
        ...
        import streamlit as st
        st.error(f"[References 로드 오류] {e}")
        
    return loaded_files


def build_gemma_memory_prompt_preview(memory_items: list, max_chars: int = 30000) -> str:
    """7차 references 로드 결과를 바탕으로 젬마 주입용 메모리 프롬프트 블록을 구성합니다."""
    import re
...
def build_manual_memory_injected_prompt(
    ...
    if len(memory_buffer) + len(user_prompt) + base_len > max_chars:
        ...
        # 사용자 질문이 잘리지 않도록 memory_buffer를 우선 절삭합니다.
...
def call_gemma_safe_memory(
    user_prompt: str,
    memory_buffer: str = "",
    max_chars: int = 40000
) -> str:
    """Gemma 호출 직전에 메모리 버퍼를 결합하여 안전하게 호출하는 래퍼 함수입니다."""
    try:
        if memory_buffer and memory_buffer.strip():
            injected_prompt = build_manual_memory_injected_prompt(
        ...
        response = call_gemma(injected_prompt)
        return response
    except Exception as e:
        return f"[SAFE_MEMORY_ERROR]\n{str(e)}"
...
def save_workspace_state():
    ...
    keys_to_save = [
        ...
        # # "p2_gemma_protocol",  # 위젯 키 충돌 방지  # 위젯 키 충돌 방지 "p2_channel_url", "p2_region", "p2_topics",
...
def render_top_panel():
    st.caption("RUNNING VERSION: v15.9.34.12")
...
def render_part2():
    ...
    # ── Part 2 프롬프트 최신값 강제 갱신 (workspace_state 캐시 오염 방지) ──
    _p2_prompts = {
        "p2_master_prompt": P2_MASTER_PROMPT_DEFAULT,
        "p2_bench_prompt": (
            ...
            "기(起) 다크심리학 훅: [SOURCE: 다크심리학 — ] 또는\n"
            "         [NEED_RESEARCH: 다크심리학 {emotion}]\n"
            "승(承) 철학: 고독→쇼펜하우어 / 그림자→융 / 의미→프랭클\n"
            "         [SOURCE: 저자, 저서명]\n"
            "전(轉) 에세이: 몽테뉴 / 인간적 솔직함 / 자기수용\n"
            "         [SOURCE: 몽테뉴, 수상록]\n"
            "결(結) 성경: 실제 구절만 / [SOURCE: 성경 — 책명 NN:NN]\n\n"
            "────────────────────────────────────────\n"
            "[출력 양식]\n"
            "────────────────────────────────────────\n"
            "## 주제: {topic}\n"
        ...
        "p2_research_prompt": (
            "# 🎬 Part 2 Alchemist [탭2: 옵시디언 융합 리서치]\n\n"
            "너는 연금술사다. 선정된 주제를 실제 영상 제작 자료로 변환하라.\n\n"
            "────────────────────────────────────────\n"
            "[STEP 1] RAG 우선 순서\n"
            "────────────────────────────────────────\n"
        ...
        "p2_plan_prompt": (
            ...
            "마지막@Protagonist 문장:\n"
            "### 📦 Part 3 전달 패킷\n"
            "핵심감정: | 다크심리학: | 철학: | 성경: | @Protagonist지침:"
        ),
    }
    for _k, _v in _p2_prompts.items():
...
if uploaded_file_dev:
    ...
    if extracted_text is not None:
        ...
        # ── 11차 추가 UI: Safe Memory Wrapper 테스트 ──
        st.markdown("---")
        safe_query = st.text_input("✏️ Safe Memory Wrapper 테스트 질문", key="p11_safe_query_in")
        if st.button("🛡️ Safe Memory Wrapper 테스트", key="p11_safe_test_btn", use_container_width=True):
            if not safe_query.strip():
                ...
                with st.spinner("Wrapper를 통한 안전 Gemma 호출 중..."):
                    response = call_gemma_safe_memory(safe_query, buffer, max_chars=40000)
                    
                # debug_safe_memory_response 세션 키 저장 허용
                st.session_state["debug_safe_memory_response"] = response
                
                st.markdown("### 🤖 Safe Memory Gemma 응답 결과")
                st.write(response)
    else:
