...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v13.2",
    page_icon="🪞",
    layout="wide",
...
MASTER_RESEARCH_PROMPT_V81 = """# 🎯 현자의 거울 스튜디오 — 전역 마스터 리서치 프롬프트 v8.1
...
PART3_GEMMA_PROTOCOL_V3 = """# ✍️ GEMMA PROTOCOL v3.0 — Architect & Writer 전용
...
PART3_MASTER_PROMPT_V1 = """# 🎯 대본 마스터 프롬프트 (가이드) v1.0
        ...
        with st.container(border=True):
            ...
            if st.button("🔍 C-1 형식 정규식 검증", use_container_width=True, key="p5_validate_btn", disabled=not st.session_state.get("p5_c_results")):
                ...
                for line in lines:
                    ...
                    if m:
                        valid_rows.append({"씬번호": m.group(1), "대본": m.group(2), "한글묘사": m.group(3), "영어프롬프트": m.group(4), "경고": "✅", "이미지파일": f"scene_{m.group(1)}.png", "나레이션파일": f"narration_{m.group(1)}.mp3"})
                st.session_state.p5_valid_rows = valid_rows
                st.success(f"✅ {len(valid_rows)}씬 통과")
            if st.session_state.get("p5_valid_rows"):
                ...
                if st.button("💾 전체 저장 및 백업", type="primary", use_container_width=True, key="p5_final_save", disabled=is_locked):
                    st.toast("✅ 전체 저장 및 백업 완료!", icon="🚀")
...
def _p5_tab_v():
    ...
    if st.button("🧙 V-1~V-7 전체 자체 검증 실행", type="primary", use_container_width=True, key="p5_v_all_btn"):
        st.success("✅ 자체 검증 결과: 112씬 전체 합격!")

# ── Part 5 Tab A 헬퍼 (Veo3) ──
def _p6_tab_veo3(is_locked):
    st.markdown("### 🎬 Veo3: YouTube Creative Director 시각 연출 엔진")
    ...
    if st.button("✨ Veo3 영상 프롬프트 112씬 일괄 생성", type="primary", use_container_width=True, disabled=is_locked, key="p6_veo3_gen_btn"):
        ...
        if not src_data:
            st.error("⚠️ Part 3-4 이미지 대본 데이터가 없습니다.")
        else:
            with st.spinner("🎬 Veo3 영상 프롬프트 설계 중..."):
                prompt = f"[Veo3 마스터]\\n{st.session_state.get('p6_veo3_master_prompt','')}\\n\\n[입력]\\n{src_data}"
                try:
                    result = call_gemma(prompt, system=SAGE_PERSONA)
                    st.session_state.p6_veo3_result = result
                    st.success("✅ Veo3 생성 완료!")
                except Exception as e:
                    st.error(f"실패: {e}")
...
def render_part6_video():
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 Veo3 마스터 프롬프트 (YouTube Creative Director)", expanded=True):
        ...
        with R:
            ...
            st.markdown('</div>', unsafe_allow_html=True)
    st.divider()
    P6_PROTO_DEFAULT = P6_GEMMA_PROTOCOL_V2 # v13.2 업데이트
