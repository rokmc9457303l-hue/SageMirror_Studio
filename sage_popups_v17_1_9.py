...
def popup_assistant():
    ...
    with tab_chat:
        ...
        # (2) 사용자 입력 처리 및 즉시 렌더링 (rerun 절대 사용 금지!)
        if prompt := st.chat_input("메시지를 입력하세요..."):
            current_model = st.session_state.popup_selected_model
            current_mode = st.session_state.get("popup_gemma_mode", "A")

            # 사용자 메시지 저장 및 화면 표시
            st.session_state.popup_history.append({"role": "user", "content": prompt, "model": current_model, "part": current_part_name, "source": "사용자 입력"})
            ...
            with chat_container.chat_message("assistant"):
                if current_mode == "A":
                    # A 모드: 초경량 시스템 프롬프트(50자)로만 호출하여 무한 로딩 방지
                    sys_ctx = "너는 현자의 거울 스튜디오의 어시스턴트다. 묻는 말에 정확하고 짧게 답하라."
                    
                    # _stream_gemma_a_mode 제너레이터 활용하여 실시간 출력
                    ...
                    with st.status("🔮 젬마 에이전트 작동 중...", expanded=True) as status_widget:
                        ...
                        try:
                            ...
                            response = f"[오류] {e}"
                            ans_placeholder.error(response)
                    st.markdown(response)
                    source_label = "에이전트 루프 v1.0"

            # 젬마 응답 저장 및 영속성 보존
            st.session_state.popup_history.append({"role": "assistant", "content": response, "model": current_model, "part": current_part_name, "source": source_label})
