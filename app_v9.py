# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's 거울 스튜디오) — Master App v9.0
================================================================
[v9.0 업데이트 사항: 2026-05-13]
- Part 5 (Image Consistency) 모듈 추가 및 나노바나나 자동화 규정서 적용
"""
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v9.0",
    page_icon="🪞",
    layout="wide",
...
def render_part2():
    ...
    with c_bench:
        with st.container(border=True):
            st.markdown("### 1️⃣ 나레이션용 대본")
            st.caption("순수 나레이션용 대본 작성 (칸 띄워쓰기 등)")
    ...
    with c_research:
        with st.container(border=True):
            ...
            st.caption("001 | 대본 | 참고 및 이미지 생성 기술적 부분")
    ...
    with c_plan:
        with st.container(border=True):
            st.markdown("### 3️⃣ 캡컷용 에셋 데이터")
            st.caption("최종 영상 조립을 위한 에셋 데이터 (추후 연동)")
