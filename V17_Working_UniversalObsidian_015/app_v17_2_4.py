...
🪞 현자의 거울 스튜디오 — Master App v17.2.4

[v17.2.4 업데이트 사항: 2026-06-12]
- 우측 리서치 범용 옵시디언 4계층 연동 (Raw/Wiki/Schema/Logs)
- 중복 저장 방지 (SHA256)
- 팝업 모듈 sage_popups_v17_2_4.py 연결
- 실행 표시 버전명 정리
...
from sage_popups_v17_2_4 import (
...
st.set_page_config(
    ...
    page_title="Sage's Mirror Studio v17.2.4",
...
def save_episode_obsidian_session(episode):
    try:
        ...
        filepath = os.path.join(session_dir, f"session_{ts}_v17.2.4.md")
...
"""
        ...
        hist_filepath = os.path.join(hist_dir, f"session_{ts}_{episode}_v17.2.4.md")
...
def render_right_gemma_panel():
    ...
    ctrl_save, ctrl_status = st.columns([1.5, 3.5])
    with ctrl_save:
            ...
            if last_success_msg:
                ...
                if type(res) is dict:
                    if res.get("status") == "success":
                        st.session_state.right_gemma_last_save_status = "성공"
                        st.session_state.right_gemma_last_save_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                        st.session_state.right_gemma_last_save_path = res.get('wiki', '')
                        st.success(f"저장 성공!\n- Raw: {res.get('raw')}\n- Wiki: {res.get('wiki')}\n- Schema: {res.get('schema')}")
                    elif res.get("status") == "duplicate":
                        st.session_state.right_gemma_last_save_status = "성공(중복)"
                        st.session_state.right_gemma_last_save_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
                        st.warning(res.get("message"))
                    else:
                        st.session_state.right_gemma_last_save_status = "실패"
                        st.session_state.right_gemma_last_save_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    with ctrl_status:
        if "right_gemma_last_save_status" in st.session_state:
            st.caption(f"최근 저장: {st.session_state.right_gemma_last_save_status} | 시각: {st.session_state.right_gemma_last_save_time}")
