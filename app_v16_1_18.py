...
🪞 현자의 거울 스튜디오 — Master App v16.1.18

[v16.1.18 업데이트 사항: 2026-05-26]
- 중복 위젯 키 대응을 위한 위젯 엔진 재설계 및 로컬 상태 동기화 강화
- Part 2 데이터 저장 버그 수정: 썸네일 기획안 텍스트 영역 저장 연동
- switch_episode() 앱 시작 시 데이터 증발 근본 원인 제거
- selectbox 레이스 컨디션 방지 패치
- 사용자 명시적 변경 시에만 저장 실행
...
def render_top_panel():
    st.caption("RUNNING VERSION: v16.1.18")
...
def render_part2():
    ...
    with c_left:
        ...
        st.text_area("젬마 프로토콜 (수정은 편집 버튼 클릭)", value=st.session_state.get("p2_gemma_protocol", ""), height=270, label_visibility="collapsed", key=f"gemma_proto_{st.session_state.get('episode_name', 'EP')}", disabled=True)
...
"""
        ...
        p2_thumb_val = st.text_area("썸네일 기획 결과 (수정 가능)", value=st.session_state.get("p2_thumbnail_plan", ""), height=195, label_visibility="collapsed", key=f"thumb_area_{st.session_state.get('episode_name', 'EP')}", disabled=is_locked)
        if p2_thumb_val != st.session_state.get("p2_thumbnail_plan", ""):
            st.session_state.p2_thumbnail_plan = p2_thumb_val
            save_workspace_state()
