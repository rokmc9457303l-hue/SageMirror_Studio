# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 — Master App v14.0
[v14.0 업데이트 사항: 2026-05-21]
- 파트 6 (나레이션 & 배경음악 및 8계정 분배), 파트 7 (숏폼 생성 CapCut Bridge), 파트 8 (캡컷 최종 조립) 핵심 연계 파이프라인 및 UI 신규 구현
- 데이터 무결성 검증을 위한 06_Video_Clips 물리 파일 스캔 및 대시보드 검증 리포트 연동
- Excel 한글 깨짐 방지용 utf-8-sig 인코딩 기반 CapCut Bridge CSV 다운로드 기능 탑재
"""
...
def render_top_panel():
    ...
    with c_db:
        if db_ok:
            st.markdown(
                '<div style="background-color:#1E293B; border: 1px solid #10B981; padding: 10px; border-radius: 6px; height: 60px; display: flex; flex-direction: column; justify-content: center;">'
                '<span style="color:#10B981; font-weight:bold; font-size:0.9em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">🟢 로컬 DB 연결</span>'
                '<span style="color:#f5e9d3; font-size:0.75em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">workspace_state.json</span>'
                '</div>', unsafe_allow_html=True
            )
        else:
            st.markdown(
                '<div style="background-color:#1E293B; border: 1px solid #EF4444; padding: 10px; border-radius: 6px; height: 60px; display: flex; flex-direction: column; justify-content: center;">'
                '<span style="color:#EF4444; font-weight:bold; font-size:0.9em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">🔴 로컬 DB 없음</span>'
                '<span style="color:#f5e9d3; font-size:0.75em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">저장 필요</span>'
                '</div>', unsafe_allow_html=True
            )
    ...
    with c_obs:
        if obs_ok:
            st.markdown('<div class="clickable-card-wrapper card-rag-btn">', unsafe_allow_html=True)
            if st.button(
            ...
            st.markdown(
                '<div style="background-color:#1E293B; border: 1px solid #F59E0B; padding: 10px; border-radius: 6px; height: 60px; display: flex; flex-direction: column; justify-content: center;">'
                '<span style="color:#F59E0B; font-weight:bold; font-size:0.9em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">🟡 옵시디언 확인필요</span>'
                '<span style="color:#f5e9d3; font-size:0.75em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">경로 미설정</span>'
                '</div>', unsafe_allow_html=True
            )
    ...
    with c_git:
        if git_ok:
            ...
            st.markdown('<div class="clickable-card-wrapper card-git-btn">', unsafe_allow_html=True)
            if st.button(
                f"🟢 GitHub 연동 (커밋 조회)\nRepo: {repo_name}",
            ...
            st.markdown(
                '<div style="background-color:#1E293B; border: 1px solid #EF4444; padding: 10px; border-radius: 6px; height: 60px; display: flex; flex-direction: column; justify-content: center;">'
                '<span style="color:#EF4444; font-weight:bold; font-size:0.9em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">🔴 Git 미연동</span>'
                '<span style="color:#f5e9d3; font-size:0.75em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">설정 변경 필요</span>'
                '</div>', unsafe_allow_html=True
            )
    ...
    s1 = "🟢" if st.session_state.get("p1_topic_selection") else "🔴"
    s2 = "🟢" if st.session_state.get("p2_planning_result") else "🔴"
    s3 = "🟢" if st.session_state.get("p34_narration_script") else "🔴"
    s4 = "🟢" if st.session_state.get("p34_image_script") else "🔴"
    s5 = "🟢" if st.session_state.get("p5_valid_rows") or st.session_state.get("p5_c_results") else "🔴"
    s6 = "🟢" if is_df_valid(st.session_state.get("p6_opal_df")) else "🔴"
    s7 = "🟢" if is_df_valid(st.session_state.get("p7_capcut_df")) else "🔴"

    flow_html = f"""
    <div style="background-color:#1E293B; padding:12px; border-radius:6px; margin-bottom:15px; border: 1px solid #d4af6a;">
        <h4 style="margin:0 0 8px 0; color:#d4af6a; font-size:1.0em;">🔗 실시간 데이터 연동 상황판</h4>
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            <div style="text-align:center;"><span style="font-size:1.1em;">{s1}</span><br><span style="font-size:0.75em; color:#f5e9d3;">1. 주제 선정</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s2}</span><br><span style="font-size:0.75em; color:#f5e9d3;">2. 기획안</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s3}</span><br><span style="font-size:0.75em; color:#f5e9d3;">3. 나레이션</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s4}</span><br><span style="font-size:0.75em; color:#f5e9d3;">4. 이미지대본</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s5}</span><br><span style="font-size:0.75em; color:#f5e9d3;">5. 씬검증</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s6}</span><br><span style="font-size:0.75em; color:#f5e9d3;">6. 오팔배분</span></div>
            <span style="color:#d4af6a; font-weight:bold;">➔</span>
            <div style="text-align:center;"><span style="font-size:1.1em;">{s7}</span><br><span style="font-size:0.75em; color:#f5e9d3;">7. 캡컷조립</span></div>
        </div>
    </div>
    """
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 마스터 프롬프트", expanded=True):
        ...
        with R:
            ...
            st.text_area("파트 마스터 프롬프트", value=prompt_val, height=300, key=f"top_pr_view_{prompt_key}", label_visibility="collapsed")
...
def render_part1():
    ...
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">📚 파트 1: 벤치마킹 & 자료조사</h3>', unsafe_allow_html=True)
...
def render_part2():
    ...
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">🎨 파트 2: 총괄기획</h3>', unsafe_allow_html=True)
...
def render_part5_image():
    c_title, c_pin, c_popup = st.columns([5, 3, 2])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">🖼️ 파트 4: 이미지 생성</h3>', unsafe_allow_html=True)
    with c_pin:
        st.markdown('<div class="pin-input-container">', unsafe_allow_html=True)
...
def render_part6_video():
    c_title, c_pin, c_popup = st.columns([5, 3, 2])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">🎥 파트 5: 영상 생성</h3>', unsafe_allow_html=True)
    with c_pin:
        st.markdown('<div class="pin-input-container">', unsafe_allow_html=True)
...
def render_part34():
    ...
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">✍️ 파트 3: 대본 작성</h3>', unsafe_allow_html=True)
...
def render_part6_opal():
    c_title, c_pin, c_popup = st.columns([5, 3, 2])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">🎵 파트 6: 나레이션 & 배경음악</h3>', unsafe_allow_html=True)
    with c_pin:
        st.markdown('<div class="pin-input-container">', unsafe_allow_html=True)
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 [나레이션 & 배경음악 마스터 프롬프트]", expanded=True):
        L, R = st.columns(2, gap="medium")
        with L:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">📚 옵시디언 규칙서</div>', unsafe_allow_html=True)
            st.text_area("옵시디언 rules p6", value=st.session_state.get("obsidian_rules", ""), height=200, key="p6_obs_rules_view", label_visibility="collapsed", disabled=True)
            if st.button("[SEARCH] 편집", key="p6_obs_edit_btn", disabled=is_locked): 
                popup_edit_obsidian()
            st.markdown('</div>', unsafe_allow_html=True)
        with R:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">🎵 나레이션 & 배경음악 마스터 프롬프트</div>', unsafe_allow_html=True)
            st.text_area("파트6마스터", value=st.session_state.get("p6_master_prompt", ""), height=200, key="p6_master_prompt_view", label_visibility="collapsed", disabled=True)
            if st.button("[EDIT] 편집", key="p6_prompt_edit_btn", disabled=is_locked): 
                popup_edit_prompt()
            st.markdown('</div>', unsafe_allow_html=True)
...
def render_part7_capcut():
    c_title, c_pin, c_popup = st.columns([5, 3, 2])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">🎬 파트 7: 숏폼 생성 (CapCut Bridge)</h3>', unsafe_allow_html=True)
    with c_pin:
        st.markdown('<div class="pin-input-container">', unsafe_allow_html=True)
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 [CapCut Bridge 마스터 프롬프트]", expanded=True):
        L, R = st.columns(2, gap="medium")
        with L:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">📚 옵시디언 규칙서</div>', unsafe_allow_html=True)
            st.text_area("옵시디언 rules p7", value=st.session_state.get("obsidian_rules", ""), height=200, key="p7_obs_rules_view", label_visibility="collapsed", disabled=True)
            if st.button("[SEARCH] 편집", key="p7_obs_edit_btn", disabled=is_locked): 
                popup_edit_obsidian()
            st.markdown('</div>', unsafe_allow_html=True)
        with R:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">🎬 CapCut Bridge 마스터 프롬프트</div>', unsafe_allow_html=True)
            st.text_area("파트7마스터", value=st.session_state.get("p7_master_prompt", ""), height=200, key="p7_master_prompt_view", label_visibility="collapsed", disabled=True)
            if st.button("[EDIT] 편집", key="p7_prompt_edit_btn", disabled=is_locked): 
                popup_edit_prompt()
            st.markdown('</div>', unsafe_allow_html=True)
...
def render_part8_dashboard():
    c_title, c_pin, c_popup = st.columns([5, 3, 2])
    with c_title:
        st.markdown('<h3 style="margin: 0; line-height: 40px; color: #d4af6a;">📊 파트 8: 캡컷 최종 조립</h3>', unsafe_allow_html=True)
    with c_pin:
        st.markdown('<div class="pin-input-container">', unsafe_allow_html=True)
    ...
    with st.expander("📋 상단 공통: 옵시디언 규칙서 및 [최종 조립 마스터 프롬프트]", expanded=True):
        L, R = st.columns(2, gap="medium")
        with L:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">📚 옵시디언 규칙서</div>', unsafe_allow_html=True)
            st.text_area("옵시디언 rules p8", value=st.session_state.get("obsidian_rules", ""), height=200, key="p8_obs_rules_view", label_visibility="collapsed", disabled=True)
            if st.button("[SEARCH] 편집", key="p8_obs_edit_btn", disabled=is_locked): 
                popup_edit_obsidian()
            st.markdown('</div>', unsafe_allow_html=True)
        with R:
            st.markdown('<div class="top-panel-card"><div class="top-panel-title">📊 최종 조립 마스터 프롬프트</div>', unsafe_allow_html=True)
            st.text_area("파트8마스터", value=st.session_state.get("p8_master_prompt", ""), height=200, key="p8_master_prompt_view", label_visibility="collapsed", disabled=True)
            if st.button("[EDIT] 편집", key="p8_prompt_edit_btn", disabled=is_locked): 
                popup_edit_prompt()
            st.markdown('</div>', unsafe_allow_html=True)
