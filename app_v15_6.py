...
def render_top_panel():
    ...
    /* 클릭 가능한 카드의 래퍼 스타일 (로컬 DB연결인 sage-stat-card의 형태를 상속함) */
    .clickable-card {
        padding: 0 !important;
        cursor: pointer;
        overflow: hidden;
    }
    .clickable-card div[data-testid="stButton"] {
    ...
    .clickable-card button {
        ...
        border: none !important;
        color: #f5e9d3 !important;
        height: 58px !important; /* 부모인 sage-stat-card의 min-height 60px 및 border 1px 고려 */
        width: 100% !important;
        text-align: left !important;
    ...
    .clickable-card button:hover {
        background-color: rgba(212, 175, 106, 0.1) !important;
    }
    ...
    with c_obs:
        if obs_ok:
            st.markdown('<div class="sage-stat-card ok clickable-card">', unsafe_allow_html=True)
            if st.button(
                f"⬤  옵시디언 RAG\nVault: {obs_name}",
    ...
    with c_sync:
        st.markdown('<div class="sage-stat-card sync clickable-card">', unsafe_allow_html=True)
        if st.button(
            "🔄 전체 즉시 동기화\nGit Push & RAG", 
            type="primary", 
            use_container_width=True, 
