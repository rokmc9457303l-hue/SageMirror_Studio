...
🪞 현자의 거울 스튜디오 — Master App v17.1.2

[v17.1.2 업데이트 사항: 2026-05-29]
- [A 시스템 복구] 젬마 어시스턴트 빠른 대화 탭 완전 복구
  - 빠른 대화: call_gemma() 1회 직접 호출 / run_agent_loop 제거 / Tavily·RAG 자동 주입 OFF
  - popup_auto_search=False, popup_use_rag=False (기본값 변경)
  - popup_gemma_mode="A" 세션 키 추가
- [A/B 분리 1차] pending_stream 처리에 A 모드(빠른 대화) / B 모드(심층 에이전트) 분기 구현
- [탭 UI 정리] 탭명 재정리: 💬 빠른 대화 / 🔎 자료 조사 / 📂 젬마 자료 업로드 / 🧠 옵시디언 저장소
- [대화 영속성] popup_history JSON 저장/복원: C:\\SageMirror_Outputs\\00_Session_States\\popup_chat_EP001.json
- [태그 버그 수정] Tavily 탭 _classify_emotion_tags → _classify_universal_tags 잔존 참조 수정
- [대화 압축 뼈대] _compress_chat_history_stub() 함수 추가 (추후 구현 예정)
