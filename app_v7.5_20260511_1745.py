# -*- coding: utf-8 -*-
"""
🪞 현자의 거울 스튜디오 (Sage's Mirror Studio) — Master App v7.5
================================================================
[v7.5 업데이트 사항: 2026-05-11 17:45]
- UI 개편: Step 1에 타겟 채널 검색과 URL 입력 통합
- UI 개편: Step 2에 3단 분석 엔진 (벤치마킹 / 자료조사 / 총괄기획) 신설
- Gemma4 맞춤 프롬프트(출처 명기 필수, 15분 영상 뼈대 설계 등) 반영
...
# =====================================================================
st.set_page_config(
    page_title="Sage's Mirror Studio v7.5",
    page_icon="🪞",
    layout="wide",
...
# =====================================================================
def render_login():
    st.markdown(f"<h1 style='text-align:center'>{APP_TITLE} <span style='color:#10B981;font-size:0.5em;'>v7.5 3-Pillar Edition</span></h1>", unsafe_allow_html=True)
...
def render_part1():
    ...
    # ---------------------------------------------------------
    # NEW Step 1. 프로토콜 및 채널 설정 통합
    # ---------------------------------------------------------
    st.subheader("🧩 Step 1. 젬마 프로토콜 및 타겟 채널 탐색/설정")
    ...
    with c_left:
        st.session_state.p1_gemma_protocol = st.text_area(
            ...
            placeholder="어린아이 같은 젬마에게 부여할 엄격한 규칙을 작성하세요...\n(예: 모든 출처를 명기하고, 답변 시 반드시 [[키워드]] 형태를 사용할 것)",
            height=300,
            disabled=is_locked
        )
        
    with c_right:
        st.markdown("##### 🔍 떡상 채널 탐색기")
        ...
        if st.session_state.p1_planning_result:
            ...
            if st.button("🔒 최종안 옵시디언 자동 백업", type="primary", use_container_width=True):
                ...
                if st.session_state.path_obsidian:
                    ...
                    md = f"""# [[{topic_title}]]
## 📌 Brief Summary
Sage Mirror Studio v7.5 총괄 시나리오 기획안
...
- **Projects/Contexts:** [[SageMirror_Production_v7.5]]
