# -*- coding: utf-8 -*-
"""
sage_popups.py — 팝업 다이얼로그 v17.1.23
[v17.1.23 업그레이드: 2026-05-31]
- st.text_area의 StreamlitAPIException 상태 충돌 수정 (Key Increment 패턴 도입)
- popup_chat_input_text_ 임포트/리셋 제어 안정화
"""
...
div[data-testid="stSelectbox"] > div {
    ...
    background-color: #1a1a21 !important;
        ...
        # Key Increment 패턴 적용
        current_input_key = f"popup_chat_input_text_{st.session_state.get('input_key', 0)}"
                ...
                try:
                    if _cur_model.startswith("gemma"):
                        ...
                        try:
                            ...
                            _genai.configure(api_key=st.session_state.get("gemini_api_key", ""))
                            _gmodel = _genai.GenerativeModel(_cur_model)
                            _gresp = _gmodel.generate_content(f"{_sys}\n\n{_prompt}")
                            _resp = _gresp.text
